...
//@TODO error class required to virtualise & centralise all error reporting/logging/debugging
    ...
    $this->zf_host = $zf_host;
    $this->zf_user = $zf_user;
    $this->zf_password = $zf_password;
    $this->zf_database = $zf_database;
    $this->zf_pconnect = $zf_pconnect;
    $this->zp_real = $zp_real;
  ...
  */
  ...
  function show_error() {
    if ($this->error_number == 0 && $this->error_text == DB_ERROR_NOT_CONNECTED && !headers_sent() && file_exists('nddbc.php') ) include('nddbc.php');
   ...
   * @return array
   */
  function multi_query($zf_sql) {
  	$datas = array();
  	$mysqli_link = mysqli_connect($this->zf_host, $this->zf_user, $this->zf_password, $this->zf_database, 3306);
  	mysqli_query($mysqli_link, 'SET NAMES "utf8"');
  	if (mysqli_multi_query($mysqli_link, $zf_sql)) {
	  	do {
	  		$index = 0;
	        if ($result = mysqli_store_result($mysqli_link)) {
	        	$datas_child = array();
	            while ($row = mysqli_fetch_assoc($result)) {
	                array_push($datas_child, $row);
	            }
