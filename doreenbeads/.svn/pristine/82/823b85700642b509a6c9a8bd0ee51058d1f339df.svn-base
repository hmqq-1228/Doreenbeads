<?php
  ...
  $oID = (int)$_GET['oID'];
  include('panadmin/'.DIR_WS_LANGUAGES . $_SESSION['language'].'/invoice.php');
  
  if (!zen_not_null($oID) || !zen_not_null($_SESSION['customer_id'])) {
  	echo TEXT_CAN_NOT_SEE; exit;
  }
  $check_status = $db->Execute('select orders_status from ' . TABLE_ORDERS . ' where orders_id = ' . $oID . ' and customers_id = ' . $_SESSION['customer_id']);
  if ($check_status->RecordCount() > 0 && $check_status->fields['orders_status'] != 1) {
  
  }else{
  	echo TEXT_CAN_NOT_SEE; exit;
  }
  
  include('panadmin/'.DIR_WS_CLASSES . 'order.php');
  $order = new order($oID);
  ...
  if (isset($_GET['action']) && $_GET['action'] == 'download'){
  	if (zen_not_null($oID)){
 		...
 		$objPHPExcel = PHPExcel_IOFactory::load("file/invoice_template.xls");
...
//		}
		...
		$objPHPExcel->getActiveSheet()->setCellValue('A' . ($addr_X_coordinate + 1), $order->customer['telephone']);
		...
		$objPHPExcel->getActiveSheet()->setCellValue('A' . ($addr_X_coordinate + 2), $order->customer['email_address']);
		
		$objPHPExcel->getActiveSheet()->getCell('A' . ($addr_X_coordinate + 2))->getHyperlink()->setUrl('mailto:' . $order->customer['email_address']);
		...
		$objPHPExcel->getActiveSheet()->setCellValue('B' . ($invoice_number_X_coordinate + 1), date('M d, Y',strtotime($order->info['date_purchased'])));		
		...
		for ($i = 0, $n = sizeof($order->products); $i < $n; $i++) {
			...
			$objPHPExcel->getActiveSheet()->setCellValue('A' . ($products_X_coordinate + 1 + $i), $qty . 'x');
			$objPHPExcel->getActiveSheet()->setCellValue('B' . ($products_X_coordinate + 1 + $i), $name);
			$objPHPExcel->getActiveSheet()->setCellValue('C' . ($products_X_coordinate + 1 + $i), $model);
			$objPHPExcel->getActiveSheet()->setCellValue('D' . ($products_X_coordinate + 1 + $i), $price);
			$objPHPExcel->getActiveSheet()->setCellValue('E' . ($products_X_coordinate + 1 + $i), $total);
			
			$objPHPExcel->getActiveSheet()->getRowDimension($products_X_coordinate + 1 + $i)->setRowHeight(50);
		}
		
		$objPHPExcel->getActiveSheet()->getStyle('A' . ($products_X_coordinate + 1) . ':A' . ($products_X_coordinate + sizeof($order->products)))->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);
		$objPHPExcel->getActiveSheet()->getStyle('D' . ($products_X_coordinate + 1) . ':E' . ($products_X_coordinate +  sizeof($order->products)))->getFont()->setBold(true);
		...
		$objPHPExcel->getActiveSheet()->getStyle('A' . ($products_X_coordinate + 1) . ':E' . ($products_X_coordinate + sizeof($order->products)))->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_TOP);
		...
		//bof subtotal area
		$subtotal_X_coordinate = $products_X_coordinate + 1 + $n;		
...
if (isset($_GET['action']) && $_GET['action'] == 'downloadPDF'){
    if (zen_not_null($oID)){
        ...
        require_once(DIR_WS_CLASSES . 'class.tcpdf.php');
        ...
        class TOC_TCPDF extends TCPDF {
            ...
            public function Header() {
                if ($this->tocpage) {
                    ...
                    $this->Image('includes/templates/cherry_zen/images/logo_invoice.jpg', '155', '6', 40, 10);
        ...
        if($orders_language == 3 || $orders_language == 6 ||$_SESSION['languages_id']== 3||$_SESSION['languages_id']== 6) {
            $font = 'stsongstdlight';
        }else{
        ...
        // Vertical alignment
        $pdf->MultiCell(180, 5, $order->customer['telephone']."\n".$order->customer['email_address']);
...
<link rel="stylesheet" type="text/css" href="invoice_stylesheet.css">
<style media=print>
.noPrint {
	display:none;
}
...
<?php if(isset($_SESSION['customer_id'])&&$_SESSION['customer_id']==$order->customer['id']){?>
<!-- body_text //-->
<table border="0" width="800px" cellspacing="0" cellpadding="2">
  <tr>
    <td><table border="0" width="100%" cellspacing="0" cellpadding="0">
      <tr>
        <td class="pageHeading"><?php echo nl2br(STORE_NAME_ADDRESS); ?></td>
        <td class="pageHeading" align="right">
          <a href="<?php echo HTTP_SERVER;?>" class="alogo"><?php echo zen_image(DIR_WS_TEMPLATE_IMAGES.'logo.jpg'); ?></a>
          <p style=" margin-top:3px;text-align:center;" ><a style="padding:0;margin:0;font-size:14px;font-style:italic;font-weight:bold;color:#404040;font-family:arial;" href="<?php echo HTTP_SERVER;?>/page.html?id=159"><?php echo TEXT_LOGO_TITLE;?></a></p>
        </td>
      </tr>
    </table></td>
  </tr>
  ...
  <tr>
    <td><table border="0" cellspacing="0" cellpadding="2">
      <tr>
        ...
        <td class="main"><?php echo date('M d, Y',strtotime($order->info['date_purchased'])); ?></td>
      </tr>
      <tr>
    ...
    for ($i = 0, $n = sizeof($order->products); $i < $n; $i++) {
      ...
      echo '        </td>' . "\n" .
           '        <td class="dataTableContent" valign="top">' . $order->products[$i]['model'] . '</td>' . "\n";
	  				...
	  				$product_each_price = $currencies->format_cl (zen_add_tax ( $order->products[$i] ['final_price'], zen_get_tax_rate ( $order->products[$i] ['tax'] ) ),true, $order->info['currency'], $order->info['currency_value'] );
      echo '        <td class="dataTableContent" valign="top" align ="right">' . $order->products[$i]['weight'] . '</td>' . "\n".
           '        <td class="dataTableContent" align="right" valign="top" width="10%"><b>' .
...
?>
      <tr>
        <td align="right" colspan="2"><table border="0" cellspacing="0" cellpadding="2">
<?php
  for ($i = 0, $n = sizeof($order->totals); $i < $n; $i++) {
...
?>
    ...
    <tr class="noPrint">
        <td align="right">
            <input type="button" value="<?php echo TEXT_DOWNLOAD_INVOICE_PDF; ?>" style="color:#00f; font-weight:bold; text-decoration:none;cursor:pointer;" onclick="window.location.href='invoice.php?action=downloadPDF&oID=<?php echo $oID;?>'" />
        </td>
    </tr>
    <tr class="noPrint">
        <td align="right">
            <input type="button" value="<?php echo TEXT_DOWNLOAD_INVOICE;?>" style="color:#00f; font-weight:bold; text-decoration:none;cursor:pointer;" onclick="window.location.href='invoice.php?action=download&oID=<?php echo $oID;?>'" />
        </td>
    </tr>
    <tr class="noPrint">
        <td align="right">
            <input id="btnPrint" type="button" value="<?php echo TEXT_PRINT;?>" onclick="javascript:window.print();" style="color:#00f; font-weight:bold; text-decoration:none;cursor:pointer!important;" />
        </td>
    </tr>
</table>
<?php }else{
