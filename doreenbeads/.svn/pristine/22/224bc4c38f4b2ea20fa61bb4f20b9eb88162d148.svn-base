...
// Create a string of all affected (sub-)categories
        ...
        $time_start = zen_db_prepare_input($_POST['time_start']);
		$time_end = zen_db_prepare_input($_POST['time_end']);
    	if ($time_start!='' && !preg_match('/^(0\d{1}|1\d{1}|2[0-3]):[0-5]\d{1}:([0-5]\d{1})$/', $time_start)){
  			$action='edit';
  			$error = true;
  			 $messageStack->add("Wrong format of start time ");  			
  		}elseif($time_start==''){
  			$time_start = '00:00:00';
  		}
    	if ($time_end!='' && !preg_match('/^(0\d{1}|1\d{1}|2[0-3]):[0-5]\d{1}:([0-5]\d{1})$/', $time_end)){
  			$action='edit';
  			$error = true;
  			 $messageStack->add("Wrong format of end time");  			
  		}elseif($time_end==''){
  			$time_end = '23:59:59';
  		}
  		$_GET['sID']=$_POST['sID'];
        ...
        $salemaker_sales_data_array = array('sale_name' => zen_db_prepare_input($_POST['name']),
                                            ...
                                            'sale_date_start' => ((zen_db_prepare_input($_POST['start']) == '') ? '0001-01-01 00:00:00' : ($_POST['start']).' '.$time_start),
                                            'sale_date_end' => ((zen_db_prepare_input($_POST['end']) == '') ? '0001-01-01 00:00:00' : (($_POST['end']).' '.$time_end)));

        if($error==true){
			//zen_redirect(zen_href_link(FILENAME_SALEMAKER, 'page=' . $_GET['page'] . '&sID=' . $_POST['sID'].'&action=edit'));
		}else{
          if ($action == 'insert') {
          	$salemaker_sales['sale_status'] = 0;
          	$salemaker_sales_data_array['sale_date_added'] = 'now()';
          	$salemaker_sales_data_array['sale_date_last_modified'] = '0001-01-01 00:00:00';
          	$salemaker_sales_data_array['sale_date_status_change'] = '0001-01-01 00:00:00';
      ...
      case 'copyconfirm':
        ...
        if (zen_not_null($newname)) {
          ...
          $sql_data_array = array('sale_id' => 'null',
                                  ...
                                  'sale_name' => $newname,
                                  'sale_date_added' => 'now()',
                                  'sale_date_last_modified' => '0001-01-01 00:00:00',
                                  'sale_date_status_change' => '0001-01-01 00:00:00',
...
?>
<script language="javascript">
var StartDate = new ctlSpiffyCalendarBox("StartDate", "sale_form", "start", "btnDate1","<?php echo (($sInfo->sale_date_start == '0001-01-01 00:00:00') ? '' : substr($sInfo->sale_date_start,0,10)); ?>",scBTNMODE_CUSTOMBLUE);
var EndDate = new ctlSpiffyCalendarBox("EndDate", "sale_form", "end", "btnDate2","<?php echo (($sInfo->sale_date_end == '0001-01-01 00:00:00') ? '' : substr($sInfo->sale_date_end,0,10)); ?>",scBTNMODE_CUSTOMBLUE);
</script>
      ...
      <tr>
        <td><table border="0" cellspacing="0" cellpadding="2">
          ...
          <tr>
            ...
            <td class="main">
            	<script language="javascript">StartDate.writeControl(); StartDate.dateFormat="<?php echo 'yyyy-MM-dd'; ?>";</script>
            	<?php echo zen_draw_input_field('time_start', (($sInfo->sale_date_start == '0001-01-01 00:00:00') ? '' : substr($sInfo->sale_date_start,11)), 'size="8"').'(Default:00:00:00)'?>
            </td>
          </tr>
          <tr>
            <td class="main"><?php echo TEXT_SALEMAKER_DATE_END; ?>&nbsp;</td>
            <td class="main">
            	<script language="javascript">EndDate.writeControl(); EndDate.dateFormat="<?php echo 'yyyy-MM-dd'; ?>";</script>
            	<?php echo zen_draw_input_field('time_end', (($sInfo->sale_date_end == '0001-01-01 00:00:00') ? '' : substr($sInfo->sale_date_end,11)), 'size="8"').'(Default:23:59:59)'?>
            </td>
          </tr>
        </table>
      </tr>
...
?>
                ...
                <td  class="dataTableContent" align="center"><?php echo (($salemaker_sales->fields['sale_date_start'] == '0001-01-01 00:00:00') ? TEXT_SALEMAKER_IMMEDIATELY : ($salemaker_sales->fields['sale_date_start'])); ?></td>
                <td  class="dataTableContent" align="center"><?php echo (($salemaker_sales->fields['sale_date_end'] == '0001-01-01 00:00:00') ? TEXT_SALEMAKER_NEVER : ($salemaker_sales->fields['sale_date_end'])); ?></td>
  ...
  switch ($action) {
    ...
    default:
      if (is_object($sInfo)) {
        ...
        $contents[] = array('text' => '' . TEXT_INFO_DATE_MODIFIED . ' ' . (($sInfo->sale_date_last_modified == '0001-01-01 00:00:00') ? TEXT_SALEMAKER_NEVER : zen_date_short($sInfo->sale_date_last_modified)));
        $contents[] = array('text' => '' . TEXT_INFO_DATE_STATUS_CHANGE . ' ' . (($sInfo->sale_date_status_change == '0001-01-01 00:00:00') ? TEXT_SALEMAKER_NEVER : zen_date_short($sInfo->sale_date_status_change)));
        ...
        $contents[] = array('text' => '<br>' . TEXT_INFO_DATE_START . ' ' . (($sInfo->sale_date_start == '0001-01-01 00:00:00') ? TEXT_SALEMAKER_IMMEDIATELY : ($sInfo->sale_date_start)));
        $contents[] = array('text' => '' . TEXT_INFO_DATE_END . ' ' . (($sInfo->sale_date_end == '0001-01-01 00:00:00') ? TEXT_SALEMAKER_NEVER : ($sInfo->sale_date_end)));
      }
      break;
  }
