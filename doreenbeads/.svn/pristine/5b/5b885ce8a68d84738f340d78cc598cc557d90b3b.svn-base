...
//    }
        $send_to_email = 'sale@doreenbeads.com';
        $send_to_name = 'Doreenbeads';
        ...
        $html_msg['EMAIL_MESSAGE_HTML'] = strip_tags($_POST['enquiry']). '<div style="color: grey;">###Contact Us###</div>';
        ...
        $messageStack->add_session('contact', EMAIL_SEND_SUCCESS, 'success');
...
//    }
        ...
        $error = true;
        if (empty($name)) {
            $messageStack->add_session('contact', ENTRY_EMAIL_NAME_CHECK_ERROR);
        }
        if ($zc_validate_email == false) {
            $messageStack->add_session('contact', ENTRY_EMAIL_ADDRESS_CHECK_ERROR);
        }
        if (empty($enquiry)) {
            $messageStack->add_session('contact', ENTRY_EMAIL_CONTENT_CHECK_ERROR);
        }
    }
...
// default email and name if customer is logged in
    ...
    if ($_SESSION['auto_auth_code_display']['contact_us'] >= 3) {
