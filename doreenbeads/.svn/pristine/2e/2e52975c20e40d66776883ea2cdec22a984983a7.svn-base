...
//echo "I AM LOADING: " . $language_page_directory . $template_dir_select . $value . '<br />1';
  require_once($language_page_directory . $template_dir_select . $value);
}

$directory_array = $template->get_template_part(DIR_WS_LANGUAGES . $_SESSION['language'] . '/', '/^'.$current_page_base . '/');
while(list ($key, $value) = each($directory_array)) {
//echo "I AM LOADING: " . $language_page_directory . $template_dir_select . $value . '<br />';
	require_once(DIR_WS_LANGUAGES . $_SESSION['language'] . '/' . $template_dir_select . $value);
}
