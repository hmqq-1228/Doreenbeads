...
if (zen_not_null($action)) {
    switch ($action) {
        ...
        case 'save':
            ...
            $cur_code = strtoupper(zen_db_prepare_input($_POST['cur_code']));
            ...
            if (!is_numeric($value) || $value <= 0) {
                $messageStack->add_session(ERROR_ADMIN_DEMO, 'error');
            ...
            if ($action == 'insert') {
                ...
                $db->Execute("update " . TABLE_CURRENCIES . " set value='" . $value . "' where currencies_id='" . $currency_id . "'");

                if ($cur_code == 'CNY') {
                    $db->Execute('update ' . TABLE_CONFIGURATION . ' set configuration_value = ' . $value . ' where configuration_key = "MODULE_SHIPPING_CHIANPOST_CURRENCY"');
                }
        ...
        case 'deleteconfirm':
            ...
            $operate_content = '网站汇率被删除，ID: ' . $currencies_id;
            zen_insert_operate_logs($_SESSION ['admin_id'], $currencies_id, $operate_content, 4);
        ...
        case 'preview':
            ...
            $new_rate_arr = array();
            $currency = $db->Execute("select currencies_id, code, title from " . TABLE_CURRENCIES . " order by title");
            $rates = quote_xe_currency();
            while (!$currency->EOF) {

                $new_rate_arr[$currency->fields['code']] = $rates[$currency->fields['code']];
                $currency->MoveNext();
            }
        ...
        case 'update':
            $server_used = CURRENCY_SERVER_BACKUP;
            zen_set_time_limit(600);
            $currency = $db->Execute("select currencies_id, code, title, value from " . TABLE_CURRENCIES);
//            $rates = quote_xe_currency();
            while (!$currency->EOF) {
                $code = $currency->fields['code'];
                $rate = $_POST['exchange_' . $code];
                if (zen_not_null($rate) && $rate > 0) {
                    ...
                    $operate_content = '网站汇率变更: from ' . $currency->fields['value'] . ' to ' . $rate;
                    if ($currency->fields['code'] == 'CNY') {
                        $db->Execute('update ' . TABLE_CONFIGURATION . ' set configuration_value = ' . $rate . ' where configuration_key = "MODULE_SHIPPING_CHIANPOST_CURRENCY"');
                    }

                    zen_insert_operate_logs($_SESSION ['admin_id'], $currency->fields['code'], $operate_content, 4);
                            ...
                            <tr>
                                    ...
                                    <table border="0" width="100%" cellspacing="0" cellpadding="2">
                                        <tr class="dataTableHeadingRow">
                                            ...
                                            <td class="dataTableHeadingContent"
                                                align="right"><?php echo TABLE_HEADING_CURRENCY_VALUE; ?></td>
                                            <?php if ($action == 'preview') { ?>
                                                <td class="dataTableHeadingContent"
                                                    align="right"><?php echo TABLE_HEADING_NEW_CURRENCY_VALUE; ?></td>
                                            <?php } ?>
                                            <td class="dataTableHeadingContent"
                                        ...
                                        <?php
                                        $currency_query_raw = "select currencies_id, title, code, symbol_left, symbol_right, decimal_point, thousands_point, decimal_places, last_updated, value from " . TABLE_CURRENCIES . " order by currencies_id desc";
                                        ...
                                        while (!$currency->EOF) {
                                            ...
                                            <td class="dataTableContent"
                                                align="right"><?php echo number_format($currency->fields['value'], 8); ?></td>
                                            <?php if ($action == 'preview') { ?>
                                                <td class="dataTableContent"
                                                    align="right" <?php if (!is_numeric($new_rate_arr[$currency->fields['code']]) || $new_rate_arr[$currency->fields['code']] <= 0) echo 'style="color:red"'; ?>>
                                                    <?php
                                                        echo number_format($new_rate_arr[$currency->fields['code']], 8);
                                                        $currency_array[$currency->fields['code']] = number_format($new_rate_arr[$currency->fields['code']], 8);
                                                    ?>
                                                </td>
                                            <?php } ?>
                                            <td class="dataTableContent"
                                                align="right"><?php if (isset($cInfo) && is_object($cInfo) && ($currency->fields['currencies_id'] == $cInfo->currencies_id)) {
                                        ...
                                        <tr>
                                            <td colspan="4">
                                                <table border="0" width="100%" cellspacing="0" cellpadding="2">
                                                    <tr>
                                                        <td class="smallText"
                                                            valign="top"><?php echo $currency_split->display_count($currency_query_numrows, MAX_DISPLAY_SEARCH_RESULTS, $_GET['page'], TEXT_DISPLAY_NUMBER_OF_CURRENCIES); ?></td>
                                                        <td class="smallText"
                                                            align="right"><?php echo $currency_split->display_links($currency_query_numrows, MAX_DISPLAY_SEARCH_RESULTS, MAX_DISPLAY_PAGE_LINKS, $_GET['page']); ?></td>
                                                    </tr>
                                                    <?php
                                                    if (empty($action)) {
                                                        ?>
                                                        <tr>
                                                            <td><?php if (CURRENCY_SERVER_PRIMARY) {
                                                                    echo '<a href="' . zen_href_link(FILENAME_CURRENCIES, 'page=' . $_GET['page'] . '&cID=' . $cInfo->currencies_id . '&action=preview') . '">' . zen_image_button('button_get_currencies.gif', IMAGE_UPDATE_CURRENCIES) . '</a>';
                                                                } ?></td>
                                                    ...
                                                    if ($action == 'preview') {
                                                        ...
                                                        <tr>
                                                            <td width="90%"
                                                                align="right"><?php if (CURRENCY_SERVER_PRIMARY) {
                                                                    echo zen_draw_form('update_form', FILENAME_CURRENCIES, '&action=update');
                                                                    foreach ($currency_array as $cur => $rate) {
                                                                        echo zen_draw_hidden_field('exchange_' . $cur, $rate);
                                                                    }
                                                                    echo '<input type="submit" value="Update Currencies">';
                                                                    echo '</form>';
                                                                } ?></td>
                                                            <td align="left"><?php echo '<a href="' . zen_href_link(FILENAME_CURRENCIES) . '">' . zen_image_button('button_cancel.gif') . '</a>'; ?></td>
                                                        </tr>
                                                        <?php
                                                    }
                                                    ?>
                                ...
                                switch ($action) {
                                    ...
                                    case 'edit':
                                        ...
                                        $contents[] = array('text' => '<br>' . TEXT_INFO_CURRENCY_CODE . '<br>' . $cInfo->code . zen_draw_hidden_field('cur_code', $cInfo->code));
                                        $contents[] = array('text' => '<br>' . TEXT_INFO_CURRENCY_SYMBOL_LEFT . '<br>' . htmlspecialchars($cInfo->symbol_left));
                                    ...
                                    default:
                                        if (is_object($cInfo)) {
                                            ...
                                            if ($cInfo->code == 'CNY') {
                                                $contents[] = array('text' => '<span style="color:red;">美元兑人民币汇率会在更新汇率时同步到Shipping模块。</span>');
                                            }
                                        }
                                        break;
                                }
