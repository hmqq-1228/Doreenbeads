		...
		foreach($box_categories_array as $i=>$categories_array){
			...
			if($box_categories_array[$i]['pic']>0){
				switch($box_categories_array[$i]['pic']){
					case 1:
						$display_category_img = ' <img src="includes/templates/cherry_zen/images/hot.png">';
						break;
					case 2:
			...
			if($categories_array['level'] == 0){
				...
				if($categories_array['id'] == intval($cPath_array[1]) && $current_category_level == 0 || $categories_array['id'] == intval($cPath_array[3]) && $is_level_3)
					...
					$liClass = '';
					if($nextlevel == 0)		//	0
						$content .= '<li class="'.$liClass.'"><a href="'.$link.'" class="morelink">'.$name.$display_category_img.'</a></li></ul></dt>';
					else if($nextlevel == 1)		//	1
						$content .= '<li class="'.$liClass.'"><a href="'.$link.'" class="morelink">'.$name.$display_category_img.'</a></li>';
					else		//	2
						$content .= '<li class="more '.$liClass.'"><a href="'.$link.'" class="morelink">'.$name.$display_category_img.'</a><div class="threemore">';
