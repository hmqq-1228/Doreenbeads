...
if (isset($_POST['action']) && ($_POST['action'] == 'process')) {
  ...
  if ($newsletter_general != $newsletter_status) {
      ...
      if ($newsletter_general == 1) {
        ...
        $event_type = 20;
        $messageStack->add_session('unsubscribed', SUCCESS_NEWSLETTER_UNSUBSCRIBED, 'success');
      }
      ...
      /*if($success_message != ''){
        $messageStack->add_session('account', $success_message, 'success');
      }*/
    } else {
        $messageStack->add_session('warning', WARNING_NEWSLETTER_UPDATE, 'warning');
    }
    zen_redirect(zen_href_link(FILENAME_ACCOUNT_NEWSLETTERS, '', 'SSL'));

/*  if ($newsletter_general != $newsletter_from_sql) {
