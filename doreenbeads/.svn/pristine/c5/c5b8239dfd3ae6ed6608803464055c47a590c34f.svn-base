...
if (isset($_GET['action']) && $_GET['action'] == "update_memcache") {
	...
	*/
	...
	while (!$result->EOF) {
		$result->fields['keyword'] = addslashes(trim($result->fields['keyword']));
		
		$num_products_count = 0;
		$memcache_key = md5(MEMCACHE_PREFIX . $result->fields['languages_id'] . $result->fields['keyword']);
	...
	$db->Execute("insert into " . TABLE_SEARCH_RELATED . "(languages_id, keyword, keyword_display, keyword_type, keyword_status, date_created) select languages_id, keyword, keyword, keyword_type, keyword_status, date_created from " . TABLE_SEARCH_AUTO_MATCH . " where keyword_type=20");
	$db->Execute("insert into " . TABLE_SEARCH_RELATED . "(languages_id, keyword, keyword_display, keyword_type, keyword_status, date_created) select languages_id, keyword, keyword, keyword_type, keyword_status, date_created from " . TABLE_SEARCH_AUTO_MATCH . " where keyword_type=10");
	$db->Execute("insert into " . TABLE_SEARCH_RELATED . "(languages_id, keyword, keyword_display, keyword_type, keyword_status, date_created) select languages_id, keyword, keyword, keyword_type, keyword_status, date_created from " . TABLE_SEARCH_AUTO_MATCH . " where keyword_type=30");
