...
if (isset($_GET['action']) && $_GET['action'] == 'addToWishlist'){
	if (isset($_SESSION['customer_id']) && $_SESSION['customer_id'] != ''){
		...
		zen_redirect(zen_href_link(FILENAME_PRODUCTS_ALL, $all_params));
	} else {
...
//��� 2010-08-31
	...
	if ( (isset($_GET['disp_order']) && $_GET['disp_order'] == '9') || (isset($_GET['sort']) && $_GET['sort'] == '21d') ){
		if (!$_SESSION['has_valid_order']) zen_redirect(zen_href_link(FILENAME_PRODUCTS_ALL));
	}
...
// display order dropdown
  ...
  $products_all_query_raw = "SELECT p.products_type, p.products_id, p.products_level, pd.products_name, p.products_image, p.products_price, p.products_tax_class_id,
                                    ...
                                    p.master_categories_id, p.products_discount_type, p.products_quantity_order_min, p.products_quantity_order_max, p.products_volume_weight
  ...
  $products_all_split = new splitPageResults($products_all_query_raw, $number_of_products_per_page);
