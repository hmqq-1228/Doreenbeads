 ...
 */
  ...
  while (!$random_specials_sidebox_product->EOF) {
    ...
    $content .= '<a href="' . zen_href_link('product_info', 'cPath=' . get_products_info_memcache($random_specials_sidebox_product->fields["master_categories_id"] , 'cPath') . '&products_id=' . $random_specials_sidebox_product->fields["products_id"]) . '">' . zen_image(DIR_WS_IMAGES . $random_specials_sidebox_product->fields['products_image'], $random_specials_sidebox_product->fields['products_name'], SMALL_IMAGE_WIDTH, SMALL_IMAGE_HEIGHT); 
