...
if ($_POST ['orderfilter'] || $_GET ['orderfilter']) {
	$check_filter = 1;
	if($_POST ['orderfilter']) $_GET ['orderfilter'] = $_POST ['orderfilter'];
	...
	if (isset ( $_POST ['ordernumber'] ) && trim ( $_POST ['ordernumber'] ) != '') {
		$ordernumber = $db->prepare_input ( $_POST ['ordernumber'] );
		if ($ordernumber != '') {
		...
		$ordernumber = $db->prepare_input ( $_GET ['ordernumber'] );
		if ($ordernumber != '') {
			$filter_order_number_str = ' and o.orders_id = "' . $ordernumber . '" ';
		}
	}
	...
	if (isset ( $_POST ['orderstatus'] ) && trim ( $_POST ['orderstatus'] ) != '') {
		$orderstatus = ( int ) $db->prepare_input ( $_POST ['orderstatus'] );
		if ($orderstatus != 99) {
			if ($orderstatus == 1) {
		...
		$orderstatus = ( int ) $db->prepare_input ( $_GET ['status_id'] );
		if ($orderstatus != 99) {
			if ($orderstatus == 1) {
				$filter_order_status_str = ' and o.orders_status in( ' . $orderstatus . ' ,42)';
			} else {
	...
	if (isset ( $_POST ['pronumber'] ) && trim ( $_POST ['pronumber'] ) != '') {
		$pronumber = $db->prepare_input ( $_POST ['pronumber'] );
		...
		$pronumber = $db->prepare_input ( $_GET ['pronumber'] );
		//$filter_pronumber_str = ' and o.orders_id = op.orders_id and op.products_model = "' . $pronumber . '" ';
		//$filter_pronumber_table_str = ', ' . TABLE_ORDERS_PRODUCTS . ' op';
	}
	if($pronumber!=''){
	...
	if (isset ( $_POST ['datestart'] ) && trim ( $_POST ['datestart'] ) != '') {
		$datestart = substr ( $_POST ['datestart'], 0, 4 ) . '-' . substr ( $_POST ['datestart'], 5, 2 ) . '-' . substr ( $_POST ['datestart'], 8, 2 ) . ' 00:00:00';
		$filter_date_start = trim ( $_POST ['datestart'] );
		$_GET ['datestart'] = $_POST ['datestart'];
	}else if (isset ( $_GET ['datestart'] ) && trim ( $_GET ['datestart'] ) != '') {
		$datestart = substr ( $_GET ['datestart'], 0, 4 ) . '-' . substr ( $_GET ['datestart'], 5, 2 ) . '-' . substr ( $_GET ['datestart'], 8, 2 ) . ' 00:00:00';
		$filter_date_start = trim ( $_GET ['datestart'] );
	}

	if (isset ( $_POST ['dateend'] ) && trim ( $_POST ['dateend'] ) != '') {
		$dateend = substr ( $_POST ['dateend'], 0, 4 ) . '-' . substr ( $_POST ['dateend'], 5, 2 ) . '-' . substr ( $_POST ['dateend'], 8, 2 ) . ' 23:59:59';
		$filter_date_end = trim ( $_POST ['dateend'] );
		$_GET ['dateend'] = $_POST ['dateend'];
	}else if (isset ( $_GET ['dateend'] ) && trim ( $_GET ['dateend'] ) != '') {
		$dateend = substr ( $_GET ['dateend'], 0, 4 ) . '-' . substr ( $_GET ['dateend'], 5, 2 ) . '-' . substr ( $_GET ['dateend'], 8, 2 ) . ' 23:59:59';
		$filter_date_end = trim ( $_GET ['dateend'] );
	}
...
/*$breadcrumb->add (NAVBAR_TITLE,zen_href_link(FILENAME_MYACCOUNT));*/
...
/*if (isset ( $_GET ['status_id'] )) {
	if ($status_id == 1 || $status_id == 42) {
		...
		$orders_status_title = TEXT_ALL_ORDERS;
...
while ( ! $order->EOF ) {
	...
	$ordersArray [$i] = array (
			...
			'date_purchased' => date ( 'M d, Y', strtotime ( $order->fields ['date_purchased'] ) ),
...
switch (( int ) $_GET ['status_id']) {
	...
	case 42:
		...
		break;
	case 2 :
		$message ['account_status_text'] = TEXT_ORDER_PROCESSING;
		break;
	case 3 :
...
$message ['account_text_result_page'] = $history_split->display_links_mobile_for_shoppingcart(3, zen_get_all_get_params(array('page', 'info','x', 'y', 'main_page')));
$message ['account_text_result_count'] =  $history_split->display_count(TEXT_DISPLAY_NUMBER_OF_ORDERS);
...
$message ['back_url'] = zen_href_link(FILENAME_MYACCOUNT);
...
$filter ['filter_status'] = $orderstatus ? $orderstatus : $status_id;
