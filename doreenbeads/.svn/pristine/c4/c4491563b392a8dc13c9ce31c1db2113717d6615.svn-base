  ...
  function before_process() {
    ...
    if (isset($_GET['referer']) && $_GET['referer'] == 'paypal') {
      ...
      if ($pdtStatus == false) {
        ...
        zen_redirect(zen_href_link(FILENAME_CHECKOUT_SUCCESS, 'auto_close=true', 'SSL'));
      } else {
      ...
      zen_redirect(zen_href_link(FILENAME_CHECKOUT_PAYMENT, 'auto_close=true', '', 'SSL'));
    }
  }
  /**
    * Checks referrer
  ...
  function after_process() {
    ...
    // PDT found order to be approved, so add a new OSH record for this order's PP details
      ...
      $sql_data_array= array(array('fieldName'=>'orders_id', 'value'=>$insert_id, 'type'=>'integer'),
                             ...
                             array('fieldName'=>'comments', 'value'=>'PayPal status(code:385): ' . $this->pdtData['payment_status'] . ' ' . ' @ ' . $this->pdtData['payment_date'] . "\n" . ' Trans ID:' . $this->pdtData['txn_id'] . "\n" . ' Amount: ' . $this->pdtData['mc_gross'] . ' ' . $this->pdtData['mc_currency'] . '.', 'type'=>'string'));
      $db->perform(TABLE_ORDERS_STATUS_HISTORY, $sql_data_array);
      
      $payment_info = json_encode(array('transaction_id' => $this->pdtData['txn_id'],'payment_method' => $this->code, 'date_created' => date('Y-m-d H:i:s')));
      $sql = "update " . TABLE_ORDERS . " set orders_status = if(orders_status = 1, :orders_status, orders_status), payment_module_code = :payment_module_code, payment_method = :payment_method, payment_info = :payment_info where orders_id = :orderID";
      $sql = $db->bindVars($sql, ':orders_status', $this->order_status, 'integer');
      $sql = $db->bindVars($sql, ':payment_module_code', $this->code, 'string');
      $sql = $db->bindVars($sql, ':payment_method', $this->title, 'string');
      $sql = $db->bindVars($sql, ':payment_info', $payment_info, 'string');
      $sql = $db->bindVars($sql, ':orderID', $insert_id, 'integer');
      $db->Execute($sql);
      ipn_debug_email('PDT NOTICE :: Order added: ' . $insert_id . "\n" . 'PayPal status(code:396): ' . $this->pdtData['payment_status'] . ' ' . ' @ ' . $this->pdtData['payment_date'] . "\n" . ' Trans ID:' . $this->pdtData['txn_id'] . "\n" . ' Amount: ' . $this->pdtData['mc_gross'] . ' ' . $this->pdtData['mc_currency']);
