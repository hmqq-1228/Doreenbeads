...
if (isset ( $_GET ['action'] ) && ($_GET ['action'] == 'process')) {
	...
	$check_customer_query = "SELECT customers_firstname, customers_lastname, customers_password, customers_facebookid
	...
	if ($check_customer->RecordCount () > 0) {
		if($check_customer->fields ['customers_password']=='' && $check_customer->fields ['customers_facebookid']!=''){	//	由facebook注册的，没有密码，不提供修改密码
			$messageStack->add_session ( 'forgetpwd', 'Oops! Our records show that you use Facebook to login, and therefore you don\'t have a password. Please use Login With Facebook, or contact us for help.', 'caution' );
			zen_redirect(zen_href_link('forgetpwd'));
		}else{
		...
		$html_msg ['EMAIL_CUSTOMERS_NAME'] = sprintf ( TEXT_DEAR , $check_customer->fields ['customers_firstname']);
		...
		$messageStack->add_session ( 'login_forgetpwd_success', sprintf(TEXT_SUCCESS_PASSWORD_SENT, $email_address), 'noImage' );
		zen_redirect ( zen_href_link ( FILENAME_FORGETPWD, '', 'SSL' ) );
		}
	} else {
		$messageStack->add_session ( 'login', TEXT_NO_EMAIL_ADDRESS_FOUND, 'noImage' );
		zen_redirect ( zen_href_link ( FILENAME_FORGETPWD, '', 'SSL' ) );
	}
}
...
$smarty->assign ('tpl','includes/templates/mobilesite/tpl/tpl_forgetpwd.html');
