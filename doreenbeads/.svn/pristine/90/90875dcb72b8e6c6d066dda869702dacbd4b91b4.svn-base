...
//   if (function_exists('session_set_cookie_params')) {
    session_set_cookie_params($lifeTime, '/', (zen_not_null($current_domain) ? $current_domain : ''));
...
// lets start our session
  zen_session_start();
  //$lifeTime   =   3   *   3600;
  setcookie(session_name(),   session_id(),   time()   +   $lifeTime,   "/ ", (zen_not_null($current_domain) ? $current_domain : ''));
