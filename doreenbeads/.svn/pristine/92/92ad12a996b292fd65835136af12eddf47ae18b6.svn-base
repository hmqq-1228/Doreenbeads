  ...
  switch ($_GET['action']) {
    ...
    case 'save':
      if ( ($_GET['lngdir']) && ($_GET['filename']) ) {
        ...
        zen_redirect(zen_href_link(FILENAME_DEFINE_PAGES_EDITOR, 'define_it=' . $_GET['define_it'] . '&action=new_page'));
      }
      break;
      case 'set_status':
  	  ...
  	  zen_redirect(zen_href_link(FILENAME_DEFINE_PAGES_EDITOR,zen_get_all_get_params(array('status','action')).'action=new_page', 'NONSSL'));
  	  break;
	  case 'set_status2':
      if(isset($_GET['status'])&&$_GET['status']!=''){
          $upate_sql='update '.TABLE_PROMOTION_COUNTDOWN.' set show_promotion_status="'.$_GET['status'].'" where languages_id="'.$_SESSION['languages_id'].'" ';
...
?>
            ...
            <td class="main"><b><?php echo TEXT_INFO_CAUTION . '<br /><br />' . TEXT_INFO_EDITING . '<br />' . $file . '<br />'; ?></b></td>
               ...
               <?php 
             if($_GET['filename']==FILENAME_DEFINE_PROMOTION.'.php'){
             ?>
             <tr>
             <td>
             Whether to display the promotional part or not?
               <?php 
				 ...
				 $countdown_info= $db->Execute($countdown_info_query);
		   		 echo ($countdown_info->fields['show_promotion_status']) ? '<a href="' . zen_href_link(FILENAME_DEFINE_PAGES_EDITOR, 'action=set_status&status=0&define_it='.$_GET['define_it']) . '">' . zen_image(DIR_WS_IMAGES . 'icon_green_on.gif') . '</a>' : '<a href="' . zen_href_link(FILENAME_DEFINE_PAGES_EDITOR, 'action=set_status&status=1&define_it='.$_GET['define_it']) . '">' . zen_image(DIR_WS_IMAGES . 'icon_red_on.gif') . '</a>'; ?> 
	             </td>
             </tr>
             <?php 
             } 
             ?>
			 <?php 
             if($_GET['filename']==FILENAME_DEFINE_PROMOTION_INDEX.'.php'){
             ?>
             <tr>
             <td>
             促销首页显示
               <?php 
	   		 echo (zen_get_configuration_key_value('DISPLAY_SHOW_PROMOTION_INDEX_STATUS')) ? '<a href="' . zen_href_link(FILENAME_DEFINE_PAGES_EDITOR, 'action=set_status2&status=0&define_it='.$_GET['define_it']) . '">' . zen_image(DIR_WS_IMAGES . 'icon_green_on.gif') . '</a>' : '<a href="' . zen_href_link(FILENAME_DEFINE_PAGES_EDITOR, 'action=set_status2&status=1&define_it='.$_GET['define_it']) . '">' . zen_image(DIR_WS_IMAGES . 'icon_red_on.gif') . '</a>'; ?> 
             </td>
             </tr>
             <?php 
             } 
             ?>
          <tr><?php echo zen_draw_form('language', FILENAME_DEFINE_PAGES_EDITOR, 'lngdir=' . $_SESSION['language'] . '&filename=' . $_GET['filename'] . '&action=save&define_it=' . $_GET['define_it']); ?>
