...
?>
<div id="colone" style="display:none;">
<div class="reportBox">
...
<div id="coltwo" style="display:none;">
<div class="reportBox">
...
<div id="colthree" style="display:none;">
<div class="reportBox">
...
<div class="reportBox">
	<div class="header"><?php //echo CUSTOMERS_DATE_OF_BIRTH . '  (' . date('m-d', time() + 2*24*60*60) . ')'; ?></div>
		<?php
			/*$date_stamp = time() + 2*24*60*60;
			$date_today = date('m-d', $date_stamp);
			//echo $date_today;
			$customers_dob_today = $db->Execute("select customers_firstname, customers_lastname, customers_dob, customers_email_address from " . TABLE_CUSTOMERS . " where customers_dob like '%" . $date_today . "%'");
			$record_count = $customers_dob_today->RecordCount();
			if ($record_count > 0){
				while (!$customers_dob_today->EOF){
					echo '              <div class="row"><span class="left">' . $customers_dob_today->fields['customers_firstname'] . ' ' . $customers_dob_today->fields['customers_lastname'] . '</span><span class="center">' . ($_SESSION['show_customer_email'] ? $customers_dob_today->fields['customers_email_address'] : $customers_dob_today->fields['customers_id']) . '</span><span class="rigth">' . substr($customers_dob_today->fields['customers_dob'], 0, 10) . '</span></div>';
					$customers_dob_today->MoveNext();
				}
			}else{
				echo '					<div class="row" style="height:20px"><span class="left">Nobody has birthday on ' . date('m-d', time() + 2*24*60*60) . '</span></div>';
			}
			echo '						<div class="row" style="height:20px"><span class="right"><a href="customers_birthday_manager.php">Send Mail</a></span></div>'*/
		?>
</div>
</div>
...
<div class="copyrightrow" style="font-size:24px; font-weight:bold;height: 500px;line-height: 500px;">欢迎使用潘朵网站后台管理系统</div>
