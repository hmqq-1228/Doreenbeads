  ...
  if ($num_products_count) {
    ...
    while (!$specials->EOF) {
      ...
      $list_box_contents[$row][$col] = array('params' => 'class="specialsListBoxContents"' . ' ' . 'style="width:' . $col_width . '%;"',
                                             'text' => '<a href="' . zen_href_link('product_info', 'cPath=' . get_category_info_memcache($specials->fields['master_categories_id'] , 'cPath') . '&products_id=' . $specials->fields['products_id']) . '">' . (($specials->fields['products_image'] == '' and PRODUCTS_IMAGE_NO_IMAGE_STATUS == 0) ? '' : '<img src="' . HTTP_IMG_SERVER . 'bmz_cache/' . get_img_size($specials->fields['products_image'], 130, 130) . '" width="110" height="110" id="anchor' . $specials->fields['products_id'] . '"></a>') . '<br /><a href="' . zen_href_link('product_info', 'cPath=' . get_products_info_memcache($specials->fields['master_categories_id'] , 'cPath') . '&products_id=' . $specials->fields['products_id']) . '">' . $specials->fields['products_name'] . '</a><br />' . $products_price);
      $col ++;
