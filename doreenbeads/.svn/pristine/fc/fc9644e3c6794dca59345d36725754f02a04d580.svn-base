...
if (isset ( $_POST ['order_id'] )) {
	...
	$_SESSION ['order_number_created'] = ( int ) $_POST ['order_id'];
...
$html_msg ['EMAIL_SUBJECT'] = (preg_match ( '/@panduo.com.cn/', $order->customer ['email_address'] ) ? '测试邮件' : '') . "（订单号: " . $_POST ['order_id'] . "）有客户提交（" . strip_tags ( $$_SESSION ['payment']->title ) . "）支付信息 注意联系客户沟通付款事宜";
$str_bottom = '<div style="background:#f9f9f9;">' . TEXT_CONTINUED_EMAIL_BOTTOM . '</div>';
$email_to_sales = STORE_OWNER_EMAIL_ADDRESS;
//$email_to_sales = 'test_panduo@163.com';
$email_from = EMAIL_FROM;
switch ($_SESSION ['payment']) {
	case 'wire' : // hsbs
		$sql_data_array = array (
				'full_name' => $db->prepare_input ( $_POST ['hsbs_yname'] ),
				'currency' => $db->prepare_input ( $_POST ['hsbs_Currency_input']?$_POST ['hsbs_Currency_input']:$_POST ['hsbs_Currency']),
				'amount' => $db->prepare_input ( $_POST ['hsbs_amout'] ),
				'payment_file' => $db->prepare_input ( $_POST ['hsbs_file'] ),
				'payment_type' => $_SESSION ['payment'],
				'payment_date' => $db->prepare_input ( $_POST ['hsbs_date'] ),
				'create_date' => 'now()',
		...
		zen_mail ( '', $email_to_sales, $html_msg ['EMAIL_SUBJECT'], "", $order->customer ['name'], $email_from, $html_msg, 'default1' );
	...
	case 'wirebc' : // (Bank of China)
		$sql_data_array = array (
				'full_name' => $db->prepare_input ( $_POST ['china_yname'] ),
				'currency' => $db->prepare_input ( $_POST ['china_Currency_input']?$_POST ['china_Currency_input']:$_POST ['china_Currency'] ),
				'amount' => $db->prepare_input ( $_POST ['china_amout'] ),
				'payment_file' => $db->prepare_input ( $_POST ['china_file'] ),
				'payment_type' => $_SESSION ['payment'],
				'payment_date' => $db->prepare_input ( $_POST ['china_date'] ),
				'create_date' => 'now()',
				'orders_id' => ( int ) $_POST ['order_id']
		);
		zen_db_perform ( TABLE_PAYMENT_RECORDS, $sql_data_array );
	...
	case 'westernunion' : // Western Union Money Transfer
		$sql_data_array = array (
				'full_name' => $db->prepare_input ( $_POST ['western_yname'] ),
				'currency' => $db->prepare_input ( $_POST ['western_Currency_input']?$_POST ['western_Currency_input']:$_POST ['western_Currency'] ),
				'amount' => $db->prepare_input ( $_POST ['western_amout'] ),
				'control_no' => $db->prepare_input ( $_POST ['western_control_no'] ),
				'payment_file' => $db->prepare_input ( $_POST ['western_file'] ),
				'create_date' => 'now()',
				'payment_type' => $_SESSION ['payment'],
				'orders_id' => ( int ) $_POST ['order_id']
		);
	...
	case 'moneygram' : // Western Union Money Transfer
		$country = zen_get_countries($_POST ['zone_country_id']);
		$country_name = $country['countries_name'];
		$sql_data_array = array (
				'full_name' => $db->prepare_input ( $_POST ['moneygram_full_name'] ),
				'address' => $country_name,
				'currency' => $db->prepare_input ( $_POST ['moneygram_Currency_input']?$_POST ['moneygram_Currency_input']:$_POST ['moneygram_Currency'] ),
				'amount' => $db->prepare_input ( $_POST ['moneygram_amout'] ),
				'control_no' => $db->prepare_input ( $_POST ['moneygram_control_no'] ),
				'payment_file' => $db->prepare_input ( $_POST ['moneygram_file'] ),
				'create_date' => 'now()',
				'payment_type' => $_SESSION ['payment'],
				'orders_id' => ( int ) $_POST ['order_id']
		);
...
$order_query = "select orders_status,delivery_company,delivery_street_address,delivery_city,delivery_postcode,delivery_state
                        from " . TABLE_ORDERS . "
                        where orders_id = '" . ( int ) $_POST ['order_id'] . "' limit 1";
$orders_status = $db->Execute ( $order_query );

if ($orders_status->fields ['orders_status'] == 1) {
	if (isset ( $_POST ['coupon'] ) && ( int ) $_POST ['coupon'] > 0) {
		$coupon_value = get_coupon_value ( $_POST ['coupon'] );
		if ($coupon_value > 0) {
			if (add_discount_amount ( $_POST ['order_id'], $coupon_value )) {
				$coupon_track_data = array (
						'cr_coupon_id' => $_POST ['coupon'],
						'cr_customers_id' => $_SESSION ['customer_id'],
						'cr_orders_id' => $_POST ['order_id'],
	...
	if ($_SESSION ['payment'] == 'paypalwpp') {
		...
		if (! $payment_modules->in_special_checkout () && $payment_modules->enableDirectPayment == false) {
			...
			zen_redirect ( zen_href_link ( FILENAME_ACCOUNT_HISTORY_INFO, 'order_id=' . ( int ) $_POST ['order_id'], 'SSL' ) );
		}
	} else {
		zen_redirect ( zen_href_link ( FILENAME_ACCOUNT_HISTORY_INFO, 'order_id=' . ( int ) $_POST ['order_id'], 'SSL' ) );
	}
