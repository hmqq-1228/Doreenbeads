          ...
          if ($_POST['copy_as'] == 'link') {
            if ($categories_id != $current_category_id) {
              ...
              if ($check->fields['total'] < '1') {
              	...
              	zen_get_parent_categories($categories_all,$categories_id);
              	array_push($categories_all,$categories_id);
              	$cPath_array = $categories_all;
              	for($j=0;$j<6;$j++){
              		if($cPath_array[$j]>0){
              			$categores_array[$j]= $cPath_array[$j];
              		}else{
              			$categores_array[$j]= 0;
              		}
              	...
              	$db->Execute("insert into " . TABLE_PRODUCTS_TO_CATEGORIES . "
                    ...
                    ".$categores_array[0].",".$categores_array[1].",".$categores_array[2].",".$categores_array[3].",".$categores_array[4]."
                    ,".$categores_array[5].")");              	
              }
            } else {
            ...
            $product = $db->Execute("select products_type,pq.products_quantity, products_model, products_image,
                                     ...
                                     from " . TABLE_PRODUCTS . " p, ".TABLE_PRODUCTS_QUANTITY." pq
                                     where p.products_id = pq.products_id
            ...
            $db->Execute("insert into " . TABLE_PRODUCTS . "
                                      (products_type, products_model, products_image,
			...
			$db->Execute("insert into " . TABLE_PRODUCTS_QUANTITY . "
                    (products_id, products_quantity)
                    values ('" . (int)$dup_products_id . "', '" . $products_quantity . "')");
			
            $description = $db->Execute("select language_id, products_name,
                                                             products_url
            ...
            $products_info = $db->Execute("select language_id,products_description from ".TABLE_PRODUCTS_INFO." where products_id = ".(int)$products_id );
            while(!$products_info->EOF){
            	$db->Execute("insert into " . TABLE_PRODUCTS_INFO . "
                                        (products_id, language_id, products_description)
                            values ('" . (int)$dup_products_id . "',
                                    '" . (int)$products_info->fields['language_id'] . "',
                                    '" . zen_db_input($products_info->fields['products_description']) . "',
                                    )");
...
/////////////////////////////////////////////////////////////////////
        ...
        elseif (isset($_POST['productcheckbox']) && isset($_POST['categories_id'])){
...
// Copy attributes to duplicate product
          ...
          if ($_POST['copy_as'] == 'link') {
            if ($categories_id != $current_category_id) {
            	...
            	$products_id_from=$val;
            	///$products_id_arr=substr($products_id_arr, 0,sizeof($products_id_arr)-2);
              $check = $db->Execute("select count(*) as total
                                     from " . TABLE_PRODUCTS_TO_CATEGORIES . "
              ...
              if ($check->fields['total'] < '1') {
                $db->Execute("insert into " . TABLE_PRODUCTS_TO_CATEGORIES . "
                                          (products_id, categories_id)
                              values ('" . (int)$val . "', '" . (int)$categories_id . "')");
              }
              //else {}
            } }else {
              $messageStack->add_session(ERROR_CANNOT_LINK_TO_SAME_CATEGORY, 'error');
            ...
            $products_info = $db->Execute("select language_id,products_description from ".TABLE_PRODUCTS_INFO." where products_id = ".(int)$val );
            while(!$products_info->EOF){
            	$db->Execute("insert into " . TABLE_PRODUCTS_INFO . "
                                        (products_id, language_id, products_description)
                            values ('" . (int)$dup_products_id . "',
...
/////////////////////////////////////////////////////////////////////
          ...
          zen_update_products_price_sorter($products_id);
           ...
           zen_redirect(zen_href_link(FILENAME_CATEGORIES, 'cPath=' . $categories_id));
        }
