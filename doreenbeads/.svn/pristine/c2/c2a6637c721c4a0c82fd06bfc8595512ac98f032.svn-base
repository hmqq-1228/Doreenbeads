...
class order_total extends base {
  ...
  /**
  * @param array 要排除的module
  */
  function process($exclude = array()) {
    global $order;
    ...
    if (is_array($this->modules)) {
      ...
      while (list(, $value) = each($this->modules)) {
        $class = substr($value, 0, strrpos($value, '.'));
        if (!isset($GLOBALS[$class]) || in_array($value, $exclude)) continue; 
        ...
        for ($i=0, $n=sizeof($GLOBALS[$class]->output); $i<$n; $i++) {
          if (zen_not_null($GLOBALS[$class]->output[$i]['title']) && zen_not_null($GLOBALS[$class]->output[$i]['text'])) {
            $order_total_array[] = array('code' => $GLOBALS[$class]->code,
                                         ...
                                         'over_arg' => $GLOBALS[$class]->output[$i]['over_arg'],
            							 'percentage_discount'=>$GLOBALS[$class]->output[$i]['percentage_discount']);
          }
        }
      }
    }
