...
function showNote(id, note){
		var $id = $j("#"+id);
		var DetC_left=$id.scrollLeft()+$id.offset().left,
			DetC_top=$id.scrollTop()+$id.offset().top,
			DetC_Box;		
		DetC_Box = "<div class='"+id+"'>"+note+"</div>";	
		$j("body").append(DetC_Box);
		$j("."+id).css({
			left : DetC_left,
			top : DetC_top,
			padding : "10px",
