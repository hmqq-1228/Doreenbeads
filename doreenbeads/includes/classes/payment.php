  ...
  */
  ...
  function javascript_validation() {
    ...
    if (is_array($this->modules) && sizeof($this->selection()) > 0) {
      ...
      '    alert(error_message);' . "\n" .
      '    return false;' . "\n" .
      '  } else if(payment_value == "paypalwpp") {' . "\n" .
      '    openPaypalWindow();' . "\n" .
      '    return false;' . "\n" .
      '  } else {' . "\n" .
      '    return true;' . "\n" .
      ...
      while (list(, $value) = each($this->modules)) {
        ...
        if ($GLOBALS[$class]->enabled) {
          ...
          if (is_array($selection)) $selection_array[] = $selection;
            ...
            //yuanzhuyuanlianwo,woyeshibibudeyi
            if($class == 'braintree'){
                $extra_payment = array(
                    array('id' => 'braintree-google', 'module' => '<strong>Google Pay</strong>'),
                    array('id' => 'braintree-apple', 'module' => '<strong>Apple Pay</strong>')
                );
                $selection_array = array_merge($selection_array, $extra_payment);
            }
        }
      }
    }
