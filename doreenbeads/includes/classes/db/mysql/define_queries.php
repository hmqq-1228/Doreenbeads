...
DEFINE('SQL_BANNER_UPDATE_CLICK_COUNT', "update " . TABLE_BANNERS_HISTORY . " set banners_clicked = banners_clicked + 1 where banners_id = '%s' and date_format(banners_history_date, '%%Y%%m%%d') = date_format(now(), '%%Y%%m%%d')");
	DEFINE('SQL_ALSO_PURCHASED', "select p.products_id, p.products_image, p.products_quantity, p.products_price, p.products_model, products_weight, p.products_quantity_order_min, p.products_quantity_order_max, p.products_volume_weight
                     from " . TABLE_ORDERS_PRODUCTS . " op, "
                            . TABLE_PRODUCTS . " p 
                     where op.products_id != '%s'
                     and orders_id in %s
