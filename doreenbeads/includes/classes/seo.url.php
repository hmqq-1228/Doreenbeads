			  ...
			  if(strstr($parameters,"language=")){
			  	...
			  	if(isset($_SESSION["languages_id"])&&$_SESSION["languages_id"]!=1){
			  		  	if(substr($parameters,(strpos($parameters,"language=")+9),2)=='en'){
      				   		$link=str_replace("/".$_SESSION["languages_code"]."/", "/", $link)  ;		  	
  						}else{
					  		$link=str_replace("/".$_SESSION["languages_code"]."/", "/".substr($parameters,(strpos($parameters,"language=")+9),2)."/", $link)  ;		  	
  						}	
...
//						case ($page == FILENAME_PRODUCT_INFO && !$this->is_attribute_string($params)):
				...
				case 'cPath':
					switch(true){
						case ($page == FILENAME_DEFAULT):
							$dest_cPath = $this->category_redirect($p2[1]);
							
							if($dest_cPath=='1') return '';
							$p2[1] = $dest_cPath;
				...
				case 'aId':
					switch(true){
						case ($page == 'subject'):
							$sql = "SELECT name FROM ".TABLE_SUBJECT_AREAS." WHERE id='".(int)($p2[1])."' LIMIT 1";
							$result = $this->db->Execute($sql);
							$name = unserialize($result->fields['name']);
							$url = $this->make_url($page, 'subject/'.$this->strip($name[1]), $p2[0], $p2[1], '.html', $separator, $page_number);
							break;
						default:
							$container[$p2[0]] = $p2[1];
							break;
	...
	function get_product_name($pID){
		switch(true){
			...
			default:
				...
				*/
			    $result = new stdClass();
			    $result->fields['products_name'] = get_products_description_memcache((int)$pID,(int)$this->languages_id);
				$pName = $this->strip($result->fields['products_name']);
			...
			default:
				switch(true){
					case ($this->attributes['SEO_ADD_CAT_PARENT'] == 'true'):
						...
						global $db,$memcache;
						$memcache_key = md5(MEMCACHE_PREFIX . 'cname' . $single_cID . $languages_id);
						...
						$result = $this->db->Execute($sql); */
						/* $sql1 = "SELECT c.categories_id,c.parent_id FROM ".TABLE_CATEGORIES." c WHERE c.categories_id = ".(int)$single_cID ;	
						$result1 = $db->Execute($sql1); */
						$parent_id = get_category_info_memcache($single_cID , 'parent_id');
						
						//$sql2 = "SELECT cd.categories_name as cName FROM ".TABLE_CATEGORIES_DESCRIPTION." cd WHERE cd.categories_id = ".(int)$single_cID." AND cd.language_id = ".(int)$this->languages_id;
						//$result2 = $db->Execute($sql2);
						
						$result2 = get_category_info_memcache((int)$single_cID, 'detail', (int)$this->languages_id);						
						//$sql3 = "SELECT cd2.categories_name as pName FROM ".TABLE_CATEGORIES_DESCRIPTION." cd2 WHERE cd2.categories_id = ".$result1->fields['parent_id']." AND cd2.language_id = ".(int)$this->languages_id ;						
						//$result3 = $db->Execute($sql3);
						
						$result3 = get_category_info_memcache($parent_id, 'detail', (int)$this->languages_id);		
						$cName = $this->not_null($result3['categories_name']) ? $result3['categories_name'] . ' ' . $result2['categories_name'] : $result2['categories_name'];
						$memcache->set($memcache_key,$cName,false,604800);
					...
					default:
						/* $sql = "SELECT categories_name as cName 
								...
								LIMIT 1";
						$result = $this->db->Execute($sql); */
						$result = get_category_info_memcache((int)$single_cID, 'detail', (int)$this->languages_id); 
						$cName = $result['categories_name'];
						break;
				}										
 ...
 */	
	function GetParentCategories(&$categories, $categories_id) {
		...
		$parent_categories  = get_category_info_memcache($categories_id , 'parent_id');
		if ($parent_categories == 0) return true;

		$categories[sizeof($categories)] = $parent_categories;

		if ($parent_categories != $categories_id) {
			$this->GetParentCategories($categories, $parent_categories);
		}
 ...
 */	
	function check_redirect(){
		...
		if(preg_match('/-c-/i', $this->uri) && $_GET['cPath']!=''){
			$new_path = $this->category_redirect($_GET['cPath']);
			if($new_path!=$_GET['cPath']){
 ...
 */	
	...
	/**
	 * Redirect deleted categories to new categoy ...
	 * @param string $category_path
	 * @author zhanghongliang 2014-1-24
	 */
	function category_redirect($category_path){
		$path_arr = explode('_', $category_path);
		$curreny_category = $path_arr[sizeof($path_arr)-1];
		//$check_redirect_query = $this->db->Execute("select new_category_id from ".TABLE_CATEGORY_REDIRECT." where old_category_id='".$curreny_category."'");
		$check_redirect_query = get_category_redirect($curreny_category);
		if($check_redirect_query->fields['new_category_id']>0){
			$new_full_cPath = $this->get_full_cPath($check_redirect_query->fields['new_category_id'], $single_cID);
			
			return $new_full_cPath;
		}else{
