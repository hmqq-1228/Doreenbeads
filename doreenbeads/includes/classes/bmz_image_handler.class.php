 ...
 */
  ...
  function is_real() {
	...
	function determine_image_sizetype() {
		...
		if (strstr($this->src, $ihConf['large']['suffix'])) {
			...
			$this->sizetype = 'small';
		} elseif ((intval($this->width) == intval($ihConf['extra']['width'])) && (intval($this->height) == intval($ihConf['extra']['height']))) {
			$this->sizetype = 'small';
		} else
	...
	function get_resized_image($width, $height, $override_sizetype = '', $filetype = '') {
      ...
      default:
		...
		if ($resize || ($this->watermark['file'] != '') || ($this->zoom['file'] != '') || ($file_extension != $this->extension)){
			$local = get_imageCacheName($this->src,$width,$height);
			//echo $local . '<br />';	
  ...
  function resize_imageGD($file_ext, $dest_name, $bg, $quality = 85) {
    ...
    imagefilledrectangle($newimg, 0, 0, $this->width - 1, $this->height - 1, $background_color);
      ...
      if ($transparent) {
        ...
        imagecolortransparent($newimg, $background_color);
		  ...
		  if($newheight > $newwidth){
			imagecopy($newimg, $tmpimg, ($this->width - $newwidth) / 2, 0, 0, 0, $this->canvas['width'], $this->canvas['height']);
		  }else{
			imagecopy($newimg, $tmpimg, 0, ($this->height - $newheight) / 2, 0, 0, $this->canvas['width'], $this->canvas['height']);
		  }
      }
    } else {
      if ($transparent) {
    ...
    if ($this->sizetype == 'small') {
      if ($ihConf[$this->sizetype]['zoom']) {
        ...
        ////jessa 2010-05-03 在此判断是否是在快速浏览功能内，如果是则调用适合快速浏览的函数
        if ($parameters == '1'){
        	return ' style="position:relative" onmouseover="showtrail_quickview(' . "'$products_image_zoom','$alt',$width,$height,$zoomwidth,$zoomheight,this," . $this->zoom['startx'].','.$this->zoom['starty'].','.$this->zoom['width'].','.$this->zoom['height'].');" onmouseout="hidetrail();" ';
        } else {
