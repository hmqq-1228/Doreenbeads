<dl class="detailinfo detailcont">
    ...
    <!-- eof -->
    <dt>
        <p class="detailtit"><strong><?php echo $products_name . ' (' . $products_model . ')'; ?></strong><input
                    type="hidden" value="<?php echo $products_id_current; ?>" class="product_id"></p>
        <p class="productview">
            <?php
            ...
            ?>
            <a rel="nofollow"
               href="<?php echo (isset($_SESSION['customer_id']) && $_SESSION['customer_id'] != '') ? 'javascript:void(0);" class="junptowritereview"' : zen_href_link(FILENAME_LOGIN, 'click=write_reviews', 'SSL') . '"'; ?>><?php echo TEXT_WRITE_REVIEWS_TITLE; ?></a>
        ...
        <?php if ((int)zen_show_discount_amount($_GET['products_id']) > 0 || get_daily_deal_price_by_products_id((int)$_GET['products_id'])) { ?>
            <p style="position: relative;top: 9px;"><span
                        style="margin-right: 5px;color: #D16369;">*</span><?php echo TEXT_DISCOUNT_PRODUCTS_TIPS; ?>
            </p>
        <?php } ?>
        <?php if ($promotion_has_max_num > 0) { ?>
            <p style="position: relative;top: 9px;"><span
                        style="margin-right: 5px;color: #D16369;">*</span><?php echo sprintf(TEXT_DISCOUNT_PRODUCTS_MAX_NUMBER_TIPS, $promotion_has_max_num); ?>
            </p>
        <?php } ?>
        ...
        <div class="detailwei">
            ...
            <?php if ($products_volume_weight > $products_weight) { ?>
                <p><a rel="nofollow" class="product_info_vol_weight" target="_blank"
                      href="index.php?main_page=page&id=21#P5"><?php echo TEXT_VOLUMETRIC_WEIGHT; ?></a>: <?php echo $products_volume_weight . ' ' . TEXT_PRODUCT_WEIGHT_UNIT; ?>
                </p>
            <?php } ?>
        ...
        <p class="fourlink1" style="line-height: 24px;">
            ...
            <?php if ($_SESSION['languages_id'] == 3) { ?>
                <a href="<?php echo 'https://vk.com/share.php?url=' . zen_href_link(FILENAME_PRODUCT_INFO, 'products_id=' . $_GET['products_id']) . '&title=' . META_TAG_TITLE . '&description=' . META_TAG_DESCRIPTION . '&image='; ?>"
                   class='st_vkontakte'><span title="<?php echo TEXT_SHARE_TO_YOUR_VK; ?>"></span></a>
            <?php } ?>
            <a href="<?php echo 'https://www.facebook.com/sharer.php?u=' . zen_href_link(FILENAME_PRODUCT_INFO, 'products_id=' . $_GET['products_id']) . '&t=' . META_TAG_TITLE ?>"
               class='st_facebook'><span title="<?php echo TEXT_SHARE_TO_YOUR_FACEBOOK; ?>"></span></a>
            <a href="<?php echo 'https://www.youtube.com/channel/UCRvI1TH9x5othhfAMts6jpQ?sub_confirmation=1' ?>"
               class='st_twitter'><span title="<?php echo TEXT_SHARE_TO_YOUR_TWITTER; ?>"></span></a>
            <a href="<?php echo 'https://www.pinterest.com/pin/create/null/?url=' . zen_href_link(FILENAME_PRODUCT_INFO, 'products_id=' . $_GET['products_id']) . '&media=' . HTTP_IMG_SERVER . "bmz_cache/images/" . get_img_size($products_image, 500, 500) . '&h=500&w=500&description=' . META_TAG_TITLE ?>"
               class='st_pinterest'><span title="<?php echo TEXT_SHARE_TO_YOUR_PINTEREST; ?>"></span></a>
            <?php if ($_SESSION['languages_id'] != 3) { ?>
                <a href="<?php echo 'https://plus.google.com/share?url=' . zen_href_link(FILENAME_PRODUCT_INFO, 'products_id=' . $_GET['products_id']) . '&hl=en-US' ?>"
                   class='st_googleplus'><span title="<?php echo TEXT_SHARE_TO_YOUR_GOOGLE; ?>"></span></a>
            <?php } ?>
        </p>
        <?php
        if ($is_silver_flag) {
...
<div class="questioncont" id="questioncont">
    <p class="questioncont_tit"><strong><?php echo TEXT_INFO_PRODUCT_QUESTION; ?></strong><span
                class="closepro">X</span></p>
    <div class="clearfix"></div>
    <div class="clearfix"></div>
    <dl class="quesform">
        <dd>
            <p class="questionimgNo">
                <img src="<?php echo HTTP_IMG_SERVER . 'bmz_cache/' . get_img_size($products_image, 130, 130) ?>"
                     width="123" height="118"/><br>
        ...
        <dt class="pq_content">
            ...
            <form>
                <table class="question-tab">
                    ...
                    <tr>
                        <td><label>*</label><?php echo ENTRY_EMAIL_ADDRESS; ?></td>
                        <td class="textinput"><input type="text" class="emailinput"
                                                     value="<?php echo $customer_email; ?>"/>
                            <p class="pq_error"></p></td>
                    </tr>
                    ...
                    <tr id="email_tr">
                        <td><label>*</label><?php echo TEXT_INFO_QUESTION; ?></td>
                        <td class="textinput"><textarea
                                    class="pq_textarea"><?php echo TEXT_INFO_PLEASE_ADD; ?></textarea>
                            <p class="pq_error"></p></td>
                    </tr>
                    <?php if ($_SESSION['auto_auth_code_display']['ask_questions'] >= 3) { ?>
                        <tr id="auth_code_tr">
                            <td style="vertical-align: middle;"><label>*</label><?php echo TEXT_VERIFY_NUMBER; ?>:</td>
                            <td class="textinput"><input type="text" class="pq_input_checkcode"
                                                         style="width:80px;margin-right:20px;"/><img id="pq_check_code"
                                                                                                     src=""
                                                                                                     onClick="this.src='./check_code.php?'+Math.random();"
                    ...
                    <tr>
                        <td></td>
                        <td><input type="submit" id="questsubbtn" value="<?php echo TEXT_SUBMIT; ?>"/></td>
                    </tr>
                </table>
            </form>
        ...
        <dd>
            <p class="questionimgNo">
                <a href="<?php echo zen_href_link(FILENAME_PRODUCT_INFO, 'products_id=' . $products_id_current) ?>">
                    <img src="<?php echo HTTP_IMG_SERVER . 'bmz_cache/' . get_img_size($products_image, 130, 130) ?>"
                         width="123" height="118"/>
                </a>
                <br>
                <?php echo $products_model; ?>
            </p>
            <p class="questiondetail"><a
                        href="<?php echo zen_href_link(FILENAME_PRODUCT_INFO, 'products_id=' . $products_id_current) ?>"><?php echo $products_name; ?></a>
            </p>
        </dd>
...
if($products_tips){
    ?>
    <div>
        <?php  echo stripslashes($products_tips).'<br/>'; ?>
    </div>
    <?php
}
?>
