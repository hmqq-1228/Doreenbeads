...
$(function () {
    ...
    $('#order_shopcart').bind('DOMNodeInserted', function () {
        lazyload($(this).find('.lazy-img'));
    });
    $('.mainwrap_l').bind('DOMNodeInserted', function () {
        lazyload($(this).find('.lazy-img'), 600);
    });
});
...
function lazyload (lazy, isFadeIn) {
    ...
    for (var i = 0, n = lazy.length; i < n; i++) {
        ...
        // 将想要加载的图片链接写入Image
        imgObj.src = lazyinfo.dataset.lazyload;
        // 使用Image的data-effect属性保存isFadeIn参数信息
        imgObj.dataset.effect = isFadeIn ? isFadeIn : '';
        
        imgObj.dataset.display = lazyinfo.dataset.display;
        
        imgObj.dataset.size = lazyinfo.dataset.size;
