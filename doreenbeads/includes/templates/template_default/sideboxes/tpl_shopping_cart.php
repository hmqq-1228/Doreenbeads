 ...
 */
  ...
  $content .= '<div id="cartBoxListWrapper">' . "\n" . '<ul>' . "\n";
    ...
    for ($i=0, $n=sizeof($products); $i<$n; $i++) {
      ...
      $content .= $products[$i]['quantity'] . BOX_SHOPPING_CART_DIVIDER . '</span><a href="' . zen_href_link('product_info', 'products_id=' . $products[$i]['id']) . '">';
...
//jessa 2009-08-10
      $content.=zen_image("images/".$products[$i]['image'], $products[$i]['name'], IMAGE_SHOPPING_CART_WIDTH, IMAGE_SHOPPING_CART_HEIGHT).'</span></a></li>' . "\n";
//eof jessa   
  ...
  if ($_SESSION['cart']->count_contents() > 100){
  	$content .= '<hr />';
  	$content .= '<div style="text-align:right;"><b><a href="' . zen_href_link(FILENAME_SHOPPING_CART) . '">More>></a></b></div>';
  }
