...
?> 
	<input type="hidden"  id="referer" size="25" name="referer" value="<?php echo $_SERVER['HTTP_REFERER'];?>"> 
<?php  
}
?>
...
<label class="inputLabel" for="login-email-address"><?php echo ENTRY_EMAIL_ADDRESS; ?></label>
<?php echo zen_draw_input_field('email_address', '', zen_set_field_length(TABLE_CUSTOMERS, 'customers_email_address', '25') . ' id="login-email-address"'); ?>
<br class="clearBoth" />

<label class="inputLabel" for="login-password"><?php echo ENTRY_PASSWORD; ?></label>
<?php echo zen_draw_password_field('password', '', zen_set_field_length(TABLE_CUSTOMERS, 'customers_password', '25') . ' id="login-password"'); ?>
<?php  
    if($_GET['main_page'] == 'login' && !isset($_GET['action'])){
?> 
	<input type="hidden"  id="referer" size="25" name="referer" value="<?php echo $_SERVER['HTTP_REFERER'];?>"> 
