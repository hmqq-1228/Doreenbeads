...
// base price
  ...
  if (strpos($ls_price, '</span><span class="productPriceDiscount">')) {
  	$ls_price = str_replace('</span><span class="productPriceDiscount">',
  		 '</span><span class="productPriceDiscountUnit">&nbsp;&nbsp;&nbsp;&nbsp;'.TEXT_PER_PACKET.'</span><span class="productPriceDiscount">', $ls_price);
  } else {
  	$ls_price = $ls_price . '<span class="productPriceDiscountUnit">&nbsp;&nbsp;&nbsp;&nbsp;'.TEXT_PER_PACKET.'</span>';
  }
...
<?php
        ...
        $the_button = PRODUCTS_ORDER_QTY_TEXT . '<input type="text" name="cart_quantity" value="' . (zen_get_buy_now_qty($_GET['products_id'])+5) . '" maxlength="6" size="2" />' . zen_get_products_quantity_min_units_display((int)$_GET['products_id']) . '' . zen_draw_hidden_field('products_id', (int)$_GET['products_id']) . '<br />' . zen_image_submit(BUTTON_IMAGE_IN_CART, BUTTON_IN_CART_ALT);
...
<div><?php echo $reviews['reviewsText'] . ($reviews['reviews_reply_text'] != '' ? '<br /><font color="#FF0000"><strong>'.TEXT_REPLY_FOR_SEASON.': </strong>' . $reviews['reviews_reply_text'] . '</font>' :'');?></div>
</div>
...
?>
	<!--jessa 2010-07-30-->
    <div class="buttonRow forward"><?php echo '<a href="' . zen_href_link(FILENAME_PRODUCT_REVIEWS_WRITE, zen_get_all_get_params(array(''))) . '">' . zen_image_button(BUTTON_IMAGE_WRITE_REVIEW, BUTTON_WRITE_REVIEW_ALT) . '</a>'; ?></div>
