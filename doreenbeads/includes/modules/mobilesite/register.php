...
if (isset($_SESSION['customer_id']) and $_SESSION['customer_id'] != '') {
	zen_redirect(zen_href_link(FILENAME_MYACCOUNT, '', 'SSL'));
}
/*
chimp_robbie wei
*/
...
/*  $account_query = "SELECT customers_gender, customers_firstname, customers_lastname,
  ...
  $account_query = $db->bindVars($account_query, ':customersID', '30782', 'integer');  
  ...
  $country_select = zen_get_country_select($country_select_name, $account->fields['customers_country_id'],$_SESSION['languages_id']);
 // var_dump($country_select);
  $smarty->assign('country_select',$country_select);
  */
  //$smarty->assign ( 'messageStack', $messageStack );
 // $smarty->assign ('tpl','includes/templates/mobilesite/tpl/tpl_register.html');
  
if (isset($_GET['action']) && ($_GET['action'] == 'process')) {
  $process = true;
  
  //$firstname = zen_db_prepare_input($_POST['reg_firstname']);
  //$lastname = zen_db_prepare_input($_POST['reg_lastname']);
  $email_address = zen_db_prepare_input($_POST['reg_email_address']);
//  $country = zen_db_prepare_input($_POST['customers_country_id']);
  $password = zen_db_prepare_input($_POST['password']);
  $confirmation = zen_db_prepare_input($_POST['confirmpass']);
  $ip_address = zen_get_ip_address();
  $agree_term = zen_db_prepare_input($_POST['agree_terms']);
 ...
 //   if ($check_email->fields['total'] > 0) {
	if ($check_email->RecordCount() > 0) {
      if($check_email->fields['customers_facebookid'] != ''){
		$error = true;
		$smarty->assign ( 'error_facebook_register', true );
	  }else{
		$error = true;
...
//     if (ACCOUNT_GENDER == 'true') $sql_data_array['customers_gender'] = $gender;
    ...
    $register_coupon_id = $db->Execute("select coupon_id from " . TABLE_COUPONS . " where coupon_code = '".REGISTER_COUPON_CODE."' limit 1")->fields['coupon_id'];
    send_register_coupon($_SESSION['customer_id'],$register_coupon_id );
    
//	include(DIR_WS_MODULES.zen_get_module_directory('share_account_register.php'));		//	add account to 8seasons
    ...
    */
    ...
    //end
    //zen_redirect(zen_href_link('myaccount', '', 'SSL'));
...
/*    if (sizeof($_SESSION['navigation']->snapshot) > 0) {
	...
	}else if(isset($_SESSION['redirect_url_after_login']) && $_SESSION['redirect_url_after_login']!=''){
		$origin_href = $_SESSION['redirect_url_after_login'];
    ...
    zen_redirect(zen_href_link(FILENAME_REGISTER_SUCCESSFULLY));
    
  } //endif !error
}else{
  zen_redirect(zen_href_link(FILENAME_LOGIN, 'is_register'));
}

$smarty->assign('get_coupon',sprintf(GET_COUPON,"US$ 6.01"));
$breadcrumb->add(NAVBAR_TITLE_REG);
$smarty->assign ('messageStack', $messageStack );
$smarty->assign ('tpl','includes/templates/mobilesite/tpl/tpl_login.html');
/*
