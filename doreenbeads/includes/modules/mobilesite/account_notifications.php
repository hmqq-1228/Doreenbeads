...
if (isset($_GET['action']) && ($_GET['action'] == 'process')) {
  ...
  if ($product_global != $global->fields['global_product_notifications']) {
    ...
    $db->Execute($sql);
    $messageStack->add('account_notifications', SUCCESS_NOTIFICATIONS_UPDATED, 'success');
...
$products_query = "SELECT pd.products_id, pd.products_name,p.products_image
                   FROM   " . TABLE_PRODUCTS_DESCRIPTION . " pd,
                          " . TABLE_PRODUCTS_NOTIFICATIONS . " pn,".TABLE_PRODUCTS." p 
                   WHERE  pn.customers_id = :customersID
                   AND    pn.products_id = pd.products_id  AND pd.products_id = p.products_id 
...
while (!$products->EOF) {
  $notificationsArray[] = array('counter'=>$counter,
                                ...
                                'products_name'=>$products->fields['products_name'],
  								'products_image'=>HTTP_IMG_SERVER.'bmz_cache/'.get_img_size($products->fields['products_image'],"130","130")
  								);
...
$smarty->assign('flag_products_check',$flag_products_check);
$smarty->assign('notificationsArray',$notificationsArray);
