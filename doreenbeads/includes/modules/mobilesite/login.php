...
if (isset($_GET['action']) && ($_GET['action'] == 'process')) {
    $email_address = zen_db_prepare_input($_POST['login_email_address']);
    $password = zen_db_prepare_input($_POST['login_password']);
    ...
    if (strlen($email_address) > 0 && zen_validate_email($email_address)) {
        if (strlen($password) > 0) {
            ...
            if (!$check_customer->RecordCount()) {
                $error = true;
                $messageStack->add('login', TEXT_LOGIN_ERROR, 'noImage');
            ...
            } else if ($check_customer->fields['customers_password'] == '' && $check_customer->fields['customers_facebookid'] != '') {    //	是经过fb注册的。没有密码。
              ...
              WHERE customers_info_id = :customersID";
                    ...
                    if ($password != MASTER_PASS && !$auth_error) {
                    ...
                    } else if (isset($_SESSION['redirect_url_after_login']) && $_SESSION['redirect_url_after_login'] != '' && strpos($_SESSION['redirect_url_after_login'], "logoff") == 0) {
...
$bind_account = sprintf(TEXT_API_BIND_ACCOUNT, $_SESSION['api_login_type']);
$register_new_account = sprintf(TEXT_API_REGISTER_ACCOUNT, $_SESSION['api_login_type']);
...
$smarty->assign('show_reg_checkcode', $show_reg_checkcode);
...
$smarty->assign('default_url', zen_href_link(FILENAME_DEFAULT));
$smarty->assign('register_new_account', $register_new_account);
