...
$p = str_replace(array('%','-'), array('+','/'), $p);
	...
	$reset_password_query = "SELECT rp_email_address,rp_modify_time,status FROM " . TABLE_RESET_PASSWORD . "
  	...
  	$smarty->assign ( 'rp_email_address',$reset_password->fields['rp_email_address']);
  	...
  	if ($reset_password->fields['status'] == 1){
	  	...
	  	if (isset($_GET['action']) && ($_GET['action'] == 'process')) {
		    ...
		    $db->Execute($sql);
		    $db->Execute('update ' . TABLE_RESET_PASSWORD . ' set rp_modify_time="'.date('Y-m-d H:i:s',$date_now).'",status=0 where rp_id='.$rp_id);
		    $messageStack->add_session('link', SUCCESS_LINK, 'noImage');
		    ...
		    $check_customer_query = "SELECT customers_id, customers_firstname, customers_password,
			  ...
			  if (!empty($_REQUEST["permLogin"])) {
				...
				$smarty->assign ('show_input', $show_input );
				$smarty->assign ('messageStack', $messageStack );
				$smarty->assign ('tpl','includes/templates/mobilesite/tpl/tpl_password_reset.html');
				echo '<script type="text/javascript">setTimeout(function(){window.location.href="'.zen_href_link(FILENAME_DEFAULT, '', 'SSL').'"}, 3000)</script>';
				
		    //eof login
		}
  	} else {
  		$messageStack->add_session('link', INVALID_LINK, 'noImage');
  		echo '<script type="text/javascript">window.location.href="'.zen_href_link(FILENAME_FORGETPWD, '', 'SSL').'"</script>';  
  	} 
