...
class ot_coupon {
  ...
  function ot_coupon() {
    ...
    $this->title = 'RCD';
    $this->title_first = MODULE_ORDER_TOTAL_COUPON_TITLE_FIRST;
  ...
  function process() {
	...
	if (zen_customer_is_new()){
		$od_amount = $this->calculate_deductions($order->info['total']);
	}else{
		if ($order->info['total'] > $order->info['promotion_total']) {
			$total = $order->info['total']-$order->info['promotion_total'];
		}else{
			$total = $order->info['subtotal']-$order->info['promotion_total'];
		}
		$od_amount = $this->calculate_deductions($total);
	}
...
////robbie_wei
////do not display the code when the code is "RCD"
	if ($this->coupon_code == '2012') {
	  			if($_SESSION['cart']->total>=200)
            	     $discount_description = '5% discount';
            	else $discount_description = '4% discount';

	  	$this->output[] = array('title' => $this->title . '(<font color="red">'.$discount_description.'</font>): ',
                              'text' => '-' . $currencies->format($od_amount['total']),
                              'value' => $od_amount['total'],
										'percentage_discount'=> $od_amount['percentage_discount']);
	  }else{
	  	if($this->coupon_code == '80214'){
			$this->output[] = array('title' => $this->title . ': ',
	                              'text' => '-' . $currencies->format($od_amount['total']),
	                              'value' => $od_amount['total'],
											'percentage_discount'=> $od_amount['percentage_discount']);
	  	} else if($this->coupon_code == 'dorabeads') {
	  		$this->output[] = array('title' => $this->title_first . ': ',
	  				'text' => '-' . $currencies->format($od_amount['total']),
	  				'value' => $od_amount['total'],
	  				'percentage_discount'=> $od_amount['percentage_discount']);
	  	} else {
   ...
   * @return integer
   */
  function customer_own_coupon(){
  	global $db;

  	////rcd_coupon_robbie_wei
  	////2008-12-23
  	$sql = "Select coupon_id From " . TABLE_COUPONS .
  		   " Where coupon_code='80214'";
  	$coupon = $db->Execute($sql);

  	$_SESSION['own_coupn'] = $coupon->fields['coupon_id'];
  	return 1;

  	$sql = "Select Count(orders_id) as orders_total From " . TABLE_ORDERS .
  			" Where customers_id = :customerID And orders_status in (" . MODULE_ORDER_PAID_VALID_STATUS_ID_GROUP . ")";

  	$sql = $db->bindVars($sql, ':customerID', $_SESSION['customer_id'], 'string');

  	$order_cnt=$db->Execute($sql);

  	if ($order_cnt->fields["orders_total"] < 1) {
  		//ebay �ʼ���ַ����
  		$sql = "Select * From zen_ebay_email, " . TABLE_CUSTOMERS . " Where ebay_email = customers_email_address And customers_id = :customerID";
  		$sql = $db->bindVars($sql, ':customerID', $_SESSION['customer_id'], 'string');
  		$ebayemail=$db->Execute($sql);

  		If ($ebayemail->EOF) {
  			$_SESSION['own_coupn'] = '1';
  		}
  		else{
  			$_SESSION['own_coupn'] = '2';
  		}
  	}
  	else{
  		$_SESSION['own_coupn'] = '2';
  ...
  function credit_selection() {
    ...
    //robbie  -090918
    //MODULE_ORDER_TOTAL_CHRISTMAS_COUPON . ' <a href="javascript:couponpopupWindow(\'' . zen_href_link(FILENAME_POPUP_COUPON_HELP, 'cID=' . $_SESSION['own_coupn']) . '\')" style="color:#0000FF">Click Here</a> to get it back.<br />' .
    //' <a href="javascript:couponpopupWindow(\'' . zen_href_link(FILENAME_POPUP_COUPON_HELP, 'cID=' . $_SESSION['own_coupn']) . '\')" style="color:#0000FF">Click Here</a> to get it back.<br />'
    $this->customer_own_coupon();
    if (!zen_customer_has_valid_order() && $order->info['subtotal'] < 30){
    	return;
    }
    $selection = array('id' => $this->code,
                   'module' => $this->title,
                   'redeem_instructions' => MODULE_ORDER_TOTAL_COUPON_REDEEM_INSTRUCTIONS .
                   MODULE_ORDER_TOTAL_COUPON_USE_NOTE . ($discount_coupon->fields['coupon_code'] != '' ? MODULE_ORDER_TOTAL_COUPON_REMOVE_INSTRUCTIONS : '') . '</p>',
...
//  rcd_coupon_robbie_wei
//	2008-12-23
//	if the customer input anything, he will get the RCD coupon
    elseif (isset($_POST['dc_redeem_code']) && $_POST['dc_redeem_code'] <> "" ) {
    	$coupon_exist = $db->Execute("Select * From " . TABLE_COUPONS . "
              where coupon_code= '" . zen_db_prepare_input($_POST['dc_redeem_code']) . "'
              and coupon_active='Y'");
    	if ($coupon_exist->RecordCount() == 0) {
    	    $special_coupon_start='2011-12-15 16:00:00';
...
//      }
    ...
    if ((isset($_POST['dc_redeem_code']) && $_POST['dc_redeem_code'] != '') || (isset($discount_coupon->fields['coupon_code']) && $discount_coupon->fields['coupon_code'] != '')) {
      ...
      if ($coupon_result->fields['coupon_type'] != 'G') {
      	...
      	//bof 老客户不能两次使用6.01
      	$coupon_valid = true;
      	/*if(zen_customer_has_valid_order() && $coupon_result->fields['coupon_amount'] == '6.01'){
      		$order_coupon_query = $db->Execute('select * from ' . TABLE_ORDERS . ' o, ' . TABLE_ORDERS_TOTAL . ' ot where o.orders_id = ot.orders_id and o.orders_status not in(0,5) and o.customers_id = ' . $_SESSION['customer_id'] . ' and ot.class = "ot_coupon" and ot.value = 6.01');
      		if ($order_coupon_query->RecordCount() > 0 || $order_total['totalFull'] < 30){
      			$coupon_valid = false;
      		}
      	}*/
      	//eof

      	if(!zen_check_first_coupon() && $coupon_result->fields['coupon_amount'] == '6.01'){
      		$coupon_valid = false;
      	}
        ...
        if ($order_total['totalFull'] < $coupon_result->fields['coupon_minimum_order']) {
          if ($coupon_result->fields['coupon_amount'] == '6.01'){
          	$messageStack->add_session('redemptions', sprintf(TEXT_INVALID_REDEEM_COUPON_MINIMUM_1, $currencies->format(6.01), $currencies->format($coupon_result->fields['coupon_minimum_order'])),'caution');
          }else{
        ...
        ////robbie 2010-11-02
    	////Ϊ������ٶȣ����еĲ�Ʒ��֧��coupon����ҪĳһЩ��Ʒ����֧�֡������Ժ��дΪ���Ч�İ취��
...
//        }
    ...
    if ($_SESSION['cc_id']) {
      ...
      if (($coupon->RecordCount() > 0 && $order_total !=0) || ($coupon->RecordCount() > 0 && $coupon->fields['coupon_type']=='S') ) {
        if ($coupon->fields['coupon_minimum_order'] <= $orderTotal['totalFull']) {
          ...
          if($coupon->fields['coupon_code']==2012){
            	if($_SESSION['cart']->total>=200){
            	$coupon->fields['coupon_amount'] = 5;
            	}
            	else{
            	$coupon->fields['coupon_amount'] = 4;
            	}
            }
            $od_amount['total'] = round($order_total*($coupon->fields['coupon_amount']/100), 2);
            $od_amount['type'] = 'P';
            $ratio = $od_amount['total']/$order_total;
				$od_amount['percentage_discount']=number_format($coupon->fields['coupon_amount'],2);
          ...
          switch ($this->calculate_tax) {
          	...
          	case 'Standard':
          		//coupon_robbie wei
          		//1.3.8 has some error change to 1.3.7
          	...
          	case 'Credit Note':
          		...
          		$od_amount['tax'] = $od_amount['total'];
          		break;
          		////end coupon_robbie wei
          	default:
          }
        }
      }
  ...
  function get_order_total() {
    ...
    //if ($this->include_shipping != 'true'&&$order->info['shipping_cost']>0) $order_total -= $order->info['shipping_cost'];
    if ($this->include_tax != 'true') $order_total -= $order->info['tax'];
    $orderTotalFull = $order_total;
    ////robbie 2010-11-02
    ////ע���������ݣ�������tax��������Լ�������couponֻ������ָ����Ʒ�����,����ִ�к���
//    $products = $_SESSION['cart']->get_products();
//    for ($i=0; $i<sizeof($products); $i++) {
...
//    $order_total = array('totalFull'=>$orderTotalFull, 'total'=>$order_total, 'tax'=>$order_total_tax);
	...
	$order_total = array('totalFull'=>$orderTotalFull, 'total'=>$order_total, 'tax'=>0);
    return $order_total;
  }
  /**
   * Enter description here...
