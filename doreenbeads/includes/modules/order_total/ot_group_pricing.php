...
class ot_group_pricing {
  ...
  function process() {
    ...
    if($_SESSION ['order_discount']) return; // 如果有设置订单折扣，则不计算vip折扣
...
//      'text' => '-' . $currencies->format($od_amount['total'], true, $order->info['currency'], $order->info['currency_value']) . ($order->info['promotion_total'] > 0 ? '<br /><div style="margin-left:-100px;color:#ff0000;">' . TEXT_PROMOTION_ITEMS . '</div>' : ''),
  ...
  function calculate_deductions($order_total) {
    ...
    if ($group_query->fields['customers_group_pricing'] != '0' || $check_channel) {
      ...
      if($check_channel){
      	$group_discount->fields['group_percentage'] = 15.00;
      }
      $gift_vouchers = $_SESSION['cart']->gv_only();
      $discount = ($order_total - $gift_vouchers) * $group_discount->fields['group_percentage'] / 100;
		$od_amount['percentage_discount']=$group_discount->fields['group_percentage'];
