 ...
 */
  function ipn_update_orders_status_and_history($ordersID, $new_status = 1, $txn_type) {
    ...
    $payment_info = addslashes(json_encode(array('transaction_id' => $_POST['txn_id'],'payment_method' => 'paypalwpp', 'date_created' => date('Y-m-d H:i:s'))));
	$db->Execute("update " . TABLE_ORDERS  . "
                    set orders_status = '" . (int)$new_status . "', payment_method='PayPal', payment_module_code='paypalwpp', payment_info = '" . $payment_info . "'
                    where orders_id = '" . (int)$ordersID . "'");

    $sql_data_array = array('orders_id' => (int)$ordersID,
                            'orders_status_id' => (int)$new_status,
                            'date_added' => 'now()',
                            'comments' => 'PayPal status(code:528): ' . $_POST['payment_status'] . ' ' . ' @ ' . $_POST['payment_date'] . (($_POST['parent_txn_id'] !='') ? "\n" . ' Parent Trans ID:' . $_POST['parent_txn_id'] : '') . "\n" . ' Trans ID:' . $_POST['txn_id'] . "\n" . ' Amount: ' . $_POST['mc_gross'] . ' ' . $_POST['mc_currency'],
