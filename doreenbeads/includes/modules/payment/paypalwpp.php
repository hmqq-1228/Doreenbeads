  ...
  function selection() {
    ...
    if ($this->enableDirectPayment == false) {
      return array('id' => $this->code,
                   'module' => '<img src="' . MODULE_PAYMENT_PAYPALWPP_MARK_BUTTON_IMG . '" alt="" /> ' . MODULE_PAYMENT_PAYPALWPP_MARK_BUTTON_TXT . '');
    }
    ...
    if (MODULE_PAYMENT_PAYPALWPP_MODULE_MODE == 'PayPal')  $fieldsArray[] = array('title' => '<br /><img src="' . MODULE_PAYMENT_PAYPALWPP_MARK_BUTTON_IMG . '" alt="" /> ' . MODULE_PAYMENT_PAYPALWPP_MARK_BUTTON_TXT . '');
...
*/
  ...
  function before_process() {
    ...
    if ($this->in_special_checkout() || $this->enableDirectPayment == false) {
    $this->zcLog('before_process - EC-1', $time."\t in ". __FILE__ . " on line " . __LINE__ . " \t\n". 'Beginning EC mode');
     /****************************************
      * Do EC checkout
      ****************************************/
      ...
      if ($_SESSION['paypal_ec_payer_info']['ship_street_1'] != '' && $_SESSION['paypal_ec_payer_info']['ship_address_status'] != 'None') {
        ...
        $this->zcLog('before_process - EC-2', $time."\t in ". __FILE__ . " on line " . __LINE__ . " \t\n". 'address overrides added:' . "\n" . print_r($options, true));
      }

      $this->zcLog('before_process - EC-3', $time."\t in ". __FILE__ . " on line " . __LINE__ . " \t\n". 'address info added:' . "\n" . print_r($options, true));
      ...
      //UAH->USD 汇率按照下单时候的汇率转换
      if($order->info['currency'] == 'UAH'){
      	$order->info['currency'] = 'USD';
      	$currency_session = $order->info['currency'];
      	$order->info['currency_value'] = 1*$order->info['currency_value']/$GLOBALS['currencies']->currencies['UAH']['value'];
      }
	  $options['CURRENCY'] = $order->info['currency'];
      $order_amount = $order->info['total']*$order->info['currency_value'];
      if($order->info['currency'] == 'JPY'){
		$order_amount = zen_round($order_amount, 0);
	  }else{
		$order_amount = zen_round($order_amount, 2);
	  }
      ...
      $this->zcLog('before_process - EC-5', $time."\t in ". __FILE__ . " on line " . __LINE__ . " \t\n". 'resultset:' . "\n" . urldecode(print_r($response, true)));
      ...
      if (empty($response['PENDINGREASON']) || 
          strtolower($response['PENDINGREASON']) == 'none' || 
          strtolower($response['PENDINGREASON']) == 'completed' || 
...
//      $this->numitems = $_SESSION['cart']->count_contents();
       ...
       * Do DP checkout
       ****************************************/
      $this->zcLog('before_process - DP-1', $time."\t in ". __FILE__ . " on line " . __LINE__ . " \t\n". 'Beginning DP mode');
      ...
      $this->zcLog('before_process - DP-2', $time."\t in ". __FILE__ . " on line " . __LINE__ . " \t\n". 'CC validation results: ' . $error . '(' . $response . ')');
      ...
      $this->zcLog('before_process - DP-3', $time."\t in ". __FILE__ . " on line " . __LINE__ . " \t\n". 'CC info: ' . $cc_validation->cc_type . ' ' . substr($cc_validation->cc_number, 0, 4) . str_repeat('X', (strlen($cc_validation->cc_number) - 8)) . substr($cc_validation->cc_number, -4));
...
//      }
      ...
      $this->zcLog('before_process - DP-5', $time."\t in ". __FILE__ . " on line " . __LINE__ . " \t\n". 'resultset:' . "\n" . print_r($response, true));
    ...
    if(!empty($this->transaction_id)) {
        $_SESSION['paypal_ec_payer_info']['transaction_id'] = $this->transaction_id;
    }
  }
  /**
  ...
  function admin_notification($zf_order_id) {
    ...
    $ipn = $db->Execute($sql);
   //bof control admin, show customer id instead of email if show_coustomer_email is false, zale 20130319
    $cid_result = $db->Execute('select customers_id from ' . TABLE_ORDERS . ' where orders_id = ' . (int)$zf_order_id);
    $cid = $cid_result->fields['customers_id'];
    //eof
  ...
  function zcLog($stage, $message) {
    ...
    if (MODULE_PAYMENT_PAYPALWPP_DEBUGGING == 'Log and Email' || MODULE_PAYMENT_PAYPALWPP_DEBUGGING == 'Log File') {
      ...
      $token .= $tokenHash;
      $date_log = date("Ymd");
      $file = $this->_logDir . '/' . $date_log . '.log';
      if (defined('PAYPAL_DEV_MODE') && PAYPAL_DEV_MODE == 'true') $file = $this->_logDir . '/' . $this->code . '_Paypal_Debug_' . $date_log . '.log';
      $fp = @fopen($file, 'a');
  ...
  function _doDebug($subject = 'PayPal debug data', $data, $useSession = true) {
    if (MODULE_PAYMENT_PAYPALWPP_DEBUGGING == 'Log and Email') {
      ...
      zen_mail(STORE_NAME, STORE_OWNER_EMAIL_ADDRESS, $subject, $data, STORE_OWNER, EMAIL_FROM, array('EMAIL_MESSAGE_HTML'=>nl2br($this->code . "\n" . $data)), 'debug');
    }
  }
  /**
...
*/
   ...
   */
  function ec_step1($orderid = '') {
    global $order, $db, $doPayPal, $currencies;
...
*/
	...
	if(isset($order->info['currency'])){
		  $options['CURRENCY'] = $order->info['currency'];
	}
    //$order_amount = $this->calc_order_amount($order->info['total'], $options['CURRENCY']);
	$order_total_query = $db->Execute('select `value` from ' . TABLE_ORDERS_TOTAL . ' where orders_id = ' . $orderid . ' and `class` = "ot_total"');
	if($order_total_query->RecordCount() > 0){
	    $order_total = $order_total_query->fields['value'];
	}else{
	    $order_total = $order->info['total'];
	}
	
	$order_amount = $order_total * $order->info['currency_value'];
    ...
    $cancel_url = zen_href_link(($_SESSION['customer_first_name'] != '' && $_SESSION['customer_id'] != '' ? FILENAME_CHECKOUT_PAYMENT :FILENAME_LOGIN), 'ec_cancel=1', 'SSL');

    // debug
    $this->zcLog('ec_step1 - 1', $time."\t in ". __FILE__ . " on line " . __LINE__ . " \t\n". 'Checking to see if we are in markflow' . "\n" . 'cart contents: ' . $_SESSION['cart']->get_content_type() . "\n\nNOTE: " . '$this->showPaymentPage = ' . (int)$this->showPaymentPage . "\nCustomer ID: " . (int)$_SESSION['customer_id'] . "\nSession Data: " . print_r($_SESSION, true));
...
/*
      if (($address_arr = $this->getOverrideAddress()) !== false) {
        $address_error = false;
        foreach(array('entry_firstname','entry_lastname','entry_street_address','entry_city','entry_postcode','zone_code','countries_iso_code_2') as $key=>$value) {
...
*/
		  ...
		  $options['ADDROVERRIDE'] = 0;
		  $address_arr = $this->getOverrideAddress();
          $options['SHIPTONAME']    = $address_arr['entry_firstname'] . ' ' . $address_arr['entry_lastname'];
          $options['SHIPTOSTREET']  = $address_arr['entry_street_address'];
          if ($address_arr['entry_suburb'] != '') $options['SHIPTOSTREET2'] = $address_arr['entry_suburb'];
          $options['SHIPTOCITY']    = $address_arr['entry_city'];
          ...
          $options['SHIPTOSTATE']   = $address_arr['zone_code'];
         // $options['SHIPTOCOUNTRYCODE'] = $address_arr['countries_iso_code_2'];
         $options['SHIPTOCOUNTRYCODE'] = $this->ec_step1_getcountrycode($address_arr['countries_iso_code_2']);
//        }
//      }

      $this->zcLog('ec-step1-addr_check3', $time."\t in ". __FILE__ . " on line " . __LINE__ . " \t\n". 'address details from override check:'.print_r($address_arr, true));
    ...
    // debug
    $this->zcLog('ec_step1 - 2 -submit', $time."\t in ". __FILE__ . " on line " . __LINE__ . " \t\n". print_r(array_merge(array('AMT' => number_format($order_amount, 2), 'RETURNURL' => $return_url, 'CANCELURL' => $cancel_url), $options), true));
  ...
  function ec_step1_getcountrycode($coutries_iso_code_2){
  	if($coutries_iso_code_2=='FX') return 'FR';
  	elseif($coutries_iso_code_2=='PR') return 'GB';
  	else return $coutries_iso_code_2;
  ...
  function ec_step2($orderid) {
    ...
    // debug
    $this->zcLog('PayPal test Log - ec_step2 $_REQUEST data', $time."\t in ". __FILE__ . " on line " . __LINE__ . " \t\n". "In function: ec_step2()\r\nData in \$_REQUEST = \r\n" . print_r($_REQUEST, true));
    ...
    $this->ec_step2_finish(array_merge($step2_payerinfo, $step2_shipto), $this->new_acct_notify, $orderid);
  }

  /**
   * Complete the step2 phase by creating accounts if needed, linking data, placing order, etc.
   */
  function ec_step2_finish($paypal_ec_payer_info, $new_acct_notify, $orderid) {
    global $db, $order;
	require(DIR_WS_CLASSES.'order.php');
	if($orderid){
	    $order=new order($orderid);
    ...
    // debug
    $this->zcLog('ec_step2_finish - 1', $time."\t in ". __FILE__ . " on line " . __LINE__ . " \t\n". 'START: paypal_ec_payer_info= ' . print_r($_SESSION['paypal_ec_payer_info'], true));
     ...
     */
    $_SESSION['paypal_ec_payer_info']['country_id'] = $country_id;
    $_SESSION['paypal_ec_payer_info']['country_code3'] = $country_code3;
    $_SESSION['paypal_ec_payer_info']['address_format_id'] = $address_format_id;
    $_SESSION['paypal_ec_payer_info']['state_id'] = $state_id;    
    // customer
...
//    }
    // debug
    $this->zcLog('ec_step2_finish - 2', $time."\t in ". __FILE__ . " on line " . __LINE__ . " \t\n". 
...
//         }
      ...
      if ($_SESSION['paypal_ec_markflow'] == 1) {
        ...
        $this->terminateEC('', false, FILENAME_CHECKOUT_PAYMENT);
      }
    } else {
      ...
      // attempt to obtain the user information using the payer_email from the info returned from PayPal, via email address paypalFLAG
      ...
      // debug
      $this->zcLog('ec_step2_finish - 4', $time."\t in ". __FILE__ . " on line " . __LINE__ . " \t\n". 'Not logged in. Looking for account.' . "\n" . $sql . "\n" . print_r($check_customer, true));
      ...
      if (!$check_customer->EOF) {
        ...
        if ($check_customer->fields['customers_paypal_ec'] == '1') {
          ...
          // debug
          $this->zcLog('ec_step2_finish - 5', $time."\t in ". __FILE__ . " on line " . __LINE__ . " \t\n". 'Found temporary account - deleting it.');

        }
      }
      //删除此逻辑 hangxiao.huang 20171123
      // Create an account, if the account does not exist 如果paypal邮箱没在网站注册过，则注册一个以paypal邮箱为用户名的账号
      //if (!$acct_exists) {

        // debug
        //$this->zcLog('ec_step2_finish - 6', $time."\t in ". __FILE__ . " on line " . __LINE__ . " \t\n". 'No ZC account found for this customer. Creating new account.' . "\n" . '$this->new_acct_notify =' . $this->new_acct_notify);
       ...
       // $sql_data_array = array();
        ...
        //write_file("log/customers_log/", "customers_firstname_" . date("Ym") . ".txt", "customers_id: " . $customer_id . "\n customers_email_address: " . $paypal_ec_payer_info['payer_email'] . "\n firstname: " . $paypal_ec_payer_info['payer_firstname'] . "\n lastname: " . $paypal_ec_payer_info['payer_lastname'] ."\n ip: " . zen_get_ip_address () ."\n WEBSITE_CODE: " . WEBSITE_CODE . "\n create_time: ". date("Y-m-d H:i:s") . "\n entrance: " . __FILE__ . " on line " . __LINE__ . "\n json_data: " . json_encode($sql_data_array) . "\n===========================================================\n\n\n");
        ...
        //if ($this->new_acct_notify == 'Yes') {
          ...
          // send the mail
          //zen_mail($paypal_ec_payer_info['payer_firstname'] . " " . $paypal_ec_payer_info['payer_lastname'], $paypal_ec_payer_info['payer_email'], EMAIL_SUBJECT, $email_text, STORE_OWNER, EMAIL_FROM, array('EMAIL_MESSAGE_HTML'=>nl2br($email_text)), 'welcome');
     ...
     // }
      ...
      // debug
      $this->zcLog('ec_step2_finish - 7', $time."\t in ". __FILE__ . " on line " . __LINE__ . " \t\n". 'Auto-Logged customer in. (' . $_SESSION['paypal_ec_payer_info']['payer_email'] . ') (' . $_SESSION['customer_id'] . ')' . "\n" . '$_SESSION[paypal_ec_temp]=' . $_SESSION['paypal_ec_temp']);
...
//       }
  ...
  function getOverrideAddress() {
    ...
    if (!empty($_GET['markflow']) && !empty($_SESSION['customer_id'])) {
      ...
      // debug
      $this->zcLog('getOverrideAddress - 1', $time."\t in ". __FILE__ . " on line " . __LINE__ . " \t\n". 'Now in markflow mode.' . "\n" . 'SESSION[sendto] = ' . $_SESSION['sendto']);
      ...
      if (!$address_arr->EOF) {
        ...
        // debug
        $this->zcLog('getOverrideAddress - 2', $time."\t in ". __FILE__ . " on line " . __LINE__ . " \t\n". '$address_arr->fields = ' . print_r($address_arr->fields, true));

        // return address data.
        return $address_arr->fields;
      }
      // debug
      $this->zcLog('getOverrideAddress - 3', $time."\t in ". __FILE__ . " on line " . __LINE__ . " \t\n". 'no override record found');
    }
    // debug
    $this->zcLog('getOverrideAddress - 4', $time."\t in ". __FILE__ . " on line " . __LINE__ . " \t\n". 'not logged in and not in markflow mode - nothing to override');
  ...
  function findMatchingAddressBookEntry($customer_id, $address_question_arr) {
      ...
      $sql = "SELECT countries_id, address_format_id
            ...
            WHERE countries_iso_code_2 = :countryId
               OR countries_name = :countryId
            LIMIT 1";
      $sql = $db->bindVars($sql, ':countryId', $address_question_arr['country_iso_code_2'], 'string');
      ...
      //     }
      // debug
      //     $this->zcLog('findMatchingAddressBookEntry - 1-stats', $time."\t in ". __FILE__ . " on line " . __LINE__ . " \t\n". 'country:' . print_r($country, true) . "\n" . 'country_zone_check:' . print_r($country_zone_check, true) . "\n" . 'zone_check:' . print_r($zone, true) . 'check_zone: ' . $check_zone . "\n" . 'zone:' . $zone_id);
      
      $city = $address_question_arr['city'];
      $street_address = $address_question_arr['street_address'];
      ...
      $sql = "SELECT address_book_id, entry_street_address, entry_suburb
                ...
                AND entry_street_address = :street_address";
      //     if ($check_zone) {
      ...
      $sql = $db->bindVars($sql, ':street_address', $street_address, 'string');
      
      $answers_arr = $db->Execute($sql);
      // debug
      $this->zcLog('findMatchingAddressBookEntry - 2-read for match', $time."\t in ". __FILE__ . " on line " . __LINE__ . " \t\n". "\nSQL was:" . $sql . "\nRecordCount = " . $answers_arr->RecordCount());
      ...
      if (!$answers_arr->EOF) {
          ...
          while (!$answers_arr->EOF) {
              ...
              // debug
              $this->zcLog('findMatchingAddressBookEntry - 3a', $time."\t in ". __FILE__ . " on line " . __LINE__ . " \t\n". "From PayPal:\r\n" . $matchQuestion . "\r\n\r\nFrom DB:\r\n" . $fromDb . "\r\n". print_r($answers_arr, true));
              ...
              if (strlen($fromDb) == strlen($matchQuestion)) {
                  if ($fromDb == $matchQuestion) {
                      ...
                      // debug
                      $this->zcLog('findMatchingAddressBookEntry - 3b', $time."\t in ". __FILE__ . " on line " . __LINE__ . " \t\n". "Exact match:\n" . print_r($answers_arr->fields, true));
      ...
      // debug
      $this->zcLog('findMatchingAddressBookEntry - 4', $time."\t in ". __FILE__ . " on line " . __LINE__ . " \t\n". "no match");
  ...
  function terminateEC($error_msg = '', $kill_sess_vars = false, $goto_page = '') {
    ...
    if ($kill_sess_vars) {
      ...
      unset($_SESSION['paypal_ec_markflow']);
      // debug
      $this->zcLog('termEC-1', $time."\t in ". __FILE__ . " on line " . __LINE__ . " \t\n". 'Killed the session vars as requested');
    }

    $this->zcLog('termEC-2', $time."\t in ". __FILE__ . " on line " . __LINE__ . " \t\n". 'BEFORE: $this->showPaymentPage = ' . (int)$this->showPaymentPage . "\nToken Data:" . $_SESSION['paypal_ec_token']);
    ...
    // debug
    $this->zcLog('termEC-3', $time."\t in ". __FILE__ . " on line " . __LINE__ . " \t\n". 'AFTER: $this->showPaymentPage = ' . (int)$this->showPaymentPage);

    if (!empty($_SESSION['customer_first_name']) && !empty($_SESSION['customer_id'])) {
      if ($this->showPaymentPage === true || $goto_page == FILENAME_CHECKOUT_PAYMENT) {
        // debug
        $this->zcLog('termEC-4', $time."\t in ". __FILE__ . " on line " . __LINE__ . " \t\n". 'We ARE logged in, and $this->showPaymentPage === true');
        ...
        //if ((!$_SESSION['shipping'] || $_SESSION['shipping'] == '') || $_SESSION['shipping']['cost'] < 0) {
          // debug
        //  $this->zcLog('termEC-5', $time."\t in ". __FILE__ . " on line " . __LINE__ . " \t\n". 'Have no shipping method selected, or shipping < 0 so set FILENAME_CHECKOUT_SHIPPING');
        //  $redirect_path = FILENAME_CHECKOUT_SHIPPING;
        //  $stackAlert = 'checkout_shipping';
        //} else {
          // debug
          $this->zcLog('termEC-6', $time."\t in ". __FILE__ . " on line " . __LINE__ . " \t\n". 'We DO have a shipping method selected, so goto PAYMENT');
    ...
    // debug
    $this->zcLog('termEC-10', $time."\t in ". __FILE__ . " on line " . __LINE__ . " \t\n". 'Redirecting to ' . $redirect_path . ' - Stack: ' . $stackAlert . "\n" . 'Message: ' . $error_msg . "\nSession Data: " . print_r($_SESSION, true));
    //zen_redirect(zen_href_link($redirect_path, 'auto_close=true&paypal_payment_message=' . $error_msg, 'SSL', true, false));
    $auto_close_str = "";
	if(empty($_SESSION['paid_by_ec'])) {
		$auto_close_str = "auto_close=true";
	}
    zen_redirect(zen_href_link($redirect_path, $auto_close_str, 'SSL', true, false));
  }
  /**
    ...
    switch($operation) {
      case 'SetExpressCheckout':
        if ($basicError) {
          ...
          if ($response['L_ERRORCODE0'] == 10401) $errorText = MODULE_PAYMENT_PAYPALWPP_TOTAL_ZERO;
...
//           if ($detailedEmailMessage != '') zen_mail(STORE_NAME, STORE_OWNER_EMAIL_ADDRESS, MODULE_PAYMENT_PAYPALWPP_TEXT_EMAIL_ERROR_SUBJECT . ' (' . $errorNum . ')', $detailedMessage, STORE_OWNER, EMAIL_FROM, array('EMAIL_MESSAGE_HTML'=>$detailedMessage), 'paymentalert');
          $this->terminateEC($errorText , true);
          return true;
        }
        break;
      ...
      case 'DoExpressCheckoutPayment':
        if ($basicError || $_SESSION['paypal_ec_token'] != urldecode($response['TOKEN'])) {
          ...
          if ($response['L_ERRORCODE0'] == 10419) $errorText = MODULE_PAYMENT_PAYPALWPP_PALPAYERID_NULL;
          if ($response['L_ERRORCODE0'] == 10486) $errorText = MODULE_PAYMENT_PAYPALWPP_NOT_PAYPALMENT;
          if ($response['L_ERRORCODE0'] == 10422) $errorText = MODULE_PAYMENT_PAYPALWPP_NOT_MONEY;
          if ($response['L_ERRORCODE0'] == 10606) $errorText = MODULE_PAYMENT_PAYPALWPP_RISK_CUSTOMER;
          if ($response['L_ERRORCODE0'] == 10736){
              $detailedMessage = MODULE_PAYMENT_PAYPALWPP_TEXT_ADDR_ERROR_FOR_SHIPPING_ADDRESS;
          }
...
//           $detailedEmailMessage = ($detailedMessage == '') ? '' : MODULE_PAYMENT_PAYPALWPP_TEXT_EMAIL_ERROR_MESSAGE . urldecode($response['L_ERRORCODE0'] . "\n" . $response['L_SHORTMESSAGE0'] . "\n" . $response['L_LONGMESSAGE0'] . $response['L_ERRORCODE1'] . "\n" . $response['L_SHORTMESSAGE1'] . "\n" . $response['L_LONGMESSAGE1'] . $response['L_ERRORCODE2'] . "\n" . $response['L_SHORTMESSAGE2'] . "\n" . $response['L_LONGMESSAGE2'] . ($response['CURL_ERRORS'] != '' ? "\n" . $response['CURL_ERRORS'] : '') . "\n\n" . 'Zen Cart message: ' . $errorText);
//           if ($detailedEmailMessage != '') zen_mail(STORE_NAME, STORE_OWNER_EMAIL_ADDRESS, MODULE_PAYMENT_PAYPALWPP_TEXT_EMAIL_ERROR_SUBJECT . ' (' . $errorNum . ')'.$_SESSION['customer_id'], $detailedMessage, STORE_OWNER, EMAIL_FROM, array('EMAIL_MESSAGE_HTML'=>$detailedMessage), 'paymentalert');
          
          $this->terminateEC(($detailedEmailMessage == '' ? $errorText : $detailedMessage), true);
          return true;
        }
        break;
...
//           if ($detailedEmailMessage != '') zen_mail(STORE_NAME, STORE_OWNER_EMAIL_ADDRESS, MODULE_PAYMENT_PAYPALWPP_TEXT_EMAIL_ERROR_SUBJECT . ' (' . $errorNum . ')'.$_SESSION['customer_id'], $detailedMessage, STORE_OWNER, EMAIL_FROM, array('EMAIL_MESSAGE_HTML'=>nl2br($detailedEmailMessage)), 'paymentalert');
      ...
      default:
        if ($basicError) {
          ...
          $detailedEmailMessage = ($detailedMessage == '') ? '' : MODULE_PAYMENT_PAYPALWPP_TEXT_EMAIL_ERROR_MESSAGE . urldecode($response['L_ERRORCODE0'] . "\n" . $response['L_SHORTMESSAGE0'] . "\n" . $response['L_LONGMESSAGE0'] . $response['L_ERRORCODE1'] . "\n" . $response['L_SHORTMESSAGE1'] . "\n" . $response['L_LONGMESSAGE1'] . $response['L_ERRORCODE2'] . "\n" . $response['L_SHORTMESSAGE2'] . "\n" . $response['L_LONGMESSAGE2'] . ($response['CURL_ERRORS'] != '' ? "\n" . $response['CURL_ERRORS'] : '') . "\n\n" . 'Zen Cart message: ' . $detailedMessage . "\n\n" . 'Transaction Response Details: ' . print_r($response, true) . "\n\n" . 'Transaction Submission: ' . urldecode($doPayPal->_sanitizeLog($doPayPal->_parseNameValueList($doPayPal->lastParamList), true)));
//           if ($detailedEmailMessage != '') zen_mail(STORE_NAME, STORE_OWNER_EMAIL_ADDRESS, MODULE_PAYMENT_PAYPALWPP_TEXT_EMAIL_ERROR_SUBJECT . ' (' . $errorNum . ')', $detailedMessage, STORE_OWNER, EMAIL_FROM, array('EMAIL_MESSAGE_HTML'=>nl2br($detailedEmailMessage)), 'paymentalert');
  ...
  /**
   * Express setexpresscheckout
   */
  function express_step1($subtotal){
  	global $db, $doPayPal;
    $time = date('Y-m-d H:i:s');
  	if (!$_SESSION['customer_id'] || !$subtotal) {
  		zen_redirect(zen_href_link(FILENAME_TIME_OUT, '', 'SSL'));
  	}else{
		$_SESSION['paypal_ec_markflow'] = 1;
	}
  	...
  	// Set currency and amount
  	$options['CURRENCY'] = $this->selectCurrency();
  	$order_amount = $this->calc_order_amount($subtotal, $options['CURRENCY']);
    if($_SESSION['currency'] == 'JPY'){
  	...
  	// Set the return URL if they click "Submit" on PayPal site
  	$return_url = zen_href_link('ipn_main_handler.php', 'action=setexpresscheckout', 'SSL', true, true, true);
  	// Select the return URL if they click "cancel" on PayPal site or click to return without making payment or login
  	$cancel_url = zen_href_link(($_SESSION['customer_first_name'] != '' && $_SESSION['customer_id'] != '' ? FILENAME_SHOPPING_CART :FILENAME_LOGIN), 'ec_cancel=1', 'SSL');
  	 
  	// debug
  	$this->zcLog('club_step1 - 1', $time."\t in ". __FILE__ . " on line " . __LINE__ . " \t\n". 'Express to see if we are in markflow' . "\nCustomer ID: " . (int)$_SESSION['customer_id'] . "\nSession Data: " . print_r($_SESSION, true));
  	 
  	// if we know customer's email address, supply it, so as to pre-fill the signup box at PayPal (for new PayPal accounts only)
  	if (!empty($_SESSION['customer_first_name']) && !empty($_SESSION['customer_id'])) {
  		$sql = "select customers_email_address,customers_telephone from " . TABLE_CUSTOMERS . " where customers_id = :custID ";
  		$sql = $db->bindVars($sql, ':custID', $_SESSION['customer_id'], 'integer');
  		$zc_getemail = $db->Execute($sql);
  		if ($zc_getemail->RecordCount() > 0 && $zc_getemail->fields['customers_email_address'] != '') {
  			$options['EMAIL'] = $zc_getemail->fields['customers_email_address'];
  	...
  	$options['SOLUTIONTYPE'] = 'SOLE';
  	// debug
  	$this->zcLog('club_step1 - 2 -submit', $time."\t in ". __FILE__ . " on line " . __LINE__ . " \t\n". print_r(array_merge(array('RETURNURL' => $return_url, 'CANCELURL' => $cancel_url), $options), true));
  	$response = $doPayPal->SetExpressCheckout(number_format($order_amount, 2), $return_url, $cancel_url, $options);
  	//print_r($response);exit;
  	/**
  	 * Determine result of request for token -- if error occurred, the errorHandler will redirect accordingly
  	*/
  	$error = $this->_errorHandler($response, 'SetExpressCheckout', '', FILENAME_CLUB_REGISTER);
