<?php
/**
 * @package languageDefines
 * @copyright Copyright 2003-2006 Zen Cart Development Team
 * @copyright Portions Copyright 2003 osCommerce
 * @license http://www.zen-cart.com/license/2_0.txt GNU Public License V2.0
 * @version $Id: account_notifications.php 3007 2006-02-11 09:17:12Z drbyte $
 */

define('NAVBAR_TITLE_1', 'My Account');
define('NAVBAR_TITLE_2', 'Email Notification');
define('NAVBAR_TITLE_3', 'Product Notifications');

define('HEADING_TITLE', 'Product Restock Notifications');

define('MY_NOTIFICATIONS_TITLE', 'My Product Notifications');
define('MY_NOTIFICATIONS_DESCRIPTION', 'This notification allows you to keep update on the product you find of interest. Once we have this product in stock, we will email to let you know instantly.');

define('GLOBAL_NOTIFICATIONS_TITLE', 'Global Product Notifications');
define('GLOBAL_NOTIFICATIONS_DESCRIPTION', 'Receive notifications on all available products.');

define('NOTIFICATIONS_TITLE', 'Product Notifications');
define('NOTIFICATIONS_DESCRIPTION', 'To remove a product notification, clear that product’s checkbox and click on Update.');
define('NOTIFICATIONS_NON_EXISTING', 'There are currently no products marked to be notified of.<br />To add products to your product notification list, click on the notification link available on the detailed product information page.');
define('TEXT_NO_PURCHASES', 'No Purchases');

define('SUCCESS_NOTIFICATIONS_UPDATED', 'Your product notifications have been updated.');
define('TEXT_UPDATE','Update')

?>