...
function submitForm(){
    ...
    var text_str1 = '';
    $("textarea[class='pDesc']").each(function(){
        //arr.push(this.value); 
         text_str1 += $(this).val();
    }); 
    // if ($.trim(text_str) == '') {alert('请至少填写一个网页端语言的标语！');return false;};

    text_str2 = '';
    $("textarea[class='pMobileDesc']").each(function(){
        //arr.push(this.value); 
         text_str2 += $(this).val();
    }); 
    if ($.trim(text_str1) == '' && $.trim(text_str2) == '') {alert('请至少填写一个网页端或手机端语言的标语！');return false;};
...
$('#canbutton').live("click",function(){
    //return false;
    window.location.href = 'tags_poster.php';
});
  // -->
</script>
<style>
    ...
    <?php   if ( isset($_GET['action'])&&('action=='.$_GET['action'])) { ?>
        ...
        if($_GET['action']=='create'){
            if (isset($_GET['anId']) && $_GET['anId'] != '') {
                ...
                $ann_desc_sql = "select an_content,an_mobile_content,an_language,an_language_name from ". TABLE_ANNOUNCEMENT_DESCRIPTION ." where an_id = ".$anid;
            ...
            <?php if (trim($anid) != '') { ?>
                ...
                <form  action="<?php echo zen_href_link(tags_poster,'action=save')?>" method="post" id="save_form">
					...
					<td class="info_column_content" > 
						<div class="index_area">
							<div class="index_language"> 
								...
								?>
									<textarea rows="9" style="width:200%;" data-lang-name="<?php echo $value['name']?>"  class="pDesc" id = "pDesc<?php echo $lang_id?>" name = "pDesc<?php echo $lang_id?>" data-id="<?php echo $lang_id;?>"><?php if ($ann_desc[$value['code']]) {echo stripslashes($ann_desc[$value['code']]['an_content']);} ?></textarea></div> 
					...
					<td class="info_column_content" > 
						<div class="index_area">
							<div class="index_language"> 
								...
								?>
									<textarea rows="9" style="width:200%;" data-lang-name="<?php echo $value['name']?>"  class="pMobileDesc" id = "pMobileDesc<?php echo $lang_id?>" name = "pMobileDesc<?php echo $lang_id?>" data-id="<?php echo $lang_id;?>"><?php if ($ann_desc[$value['code']]) {echo stripslashes($ann_desc[$value['code']]['an_mobile_content']);} ?></textarea></div> 
            ...
            <div class="buttondiv">
                ...
                <button id="canbutton" style="cursor: pointer;">取消</button>
            </div>
        <?php  }else{  ?>
            <?php 
            if($_GET['action'] == 'save'){//var_dump($_POST);
                ...
                $status = $_POST['status'];
                if ($an_name == '' || $start_time == '' || $end_time == '' || $status == '' ) {
                  zen_redirect(zen_href_link('tags_poster',zen_get_all_get_params(array('action', 'id'))));
                  exit;
                }
                $update = $_POST['update'];
                $an_text = array();
                ...
                foreach ($langs as $lang) {
                    ...
                    $an_text[$key] = isset($_POST['pDesc'.$lang['id']]) ? $_POST['pDesc'.$lang['id']] : '';
                    $an_mobile_text[$key] = isset($_POST['pMobileDesc'.$lang['id']]) ? $_POST['pMobileDesc'.$lang['id']] : '';
                    
                    if (trim($_POST['pDesc'.$lang['id']]) != '' || $_POST['pMobileDesc'.$lang['id']]) {
                    ...
                    foreach ($an_text as $lang_id => $text) {
                        ...
                        if ($text != '' || $an_mobile_text[$lang_id] != '') {
                            ...
                            if($db->Execute($sql_str)->RecordCount() > 0){
                                $db->Execute("update ".TABLE_ANNOUNCEMENT_DESCRIPTION." set an_language=".$lid.",an_language_name='".$lang_code."',an_content='".addslashes($text)."', an_mobile_content='" . addslashes($an_mobile_text[$lang_id]) . "', modify_time='".date('Y-m-d H:i:s',time())."',admin_id='".$_SESSION['admin_id']."',admin_email='".$admin_email."' where an_id=".$_POST['an_id'] ." and an_language= ".$lid);
                            }else{
    ...
    <table border="0" width="100%" cellspacing="0" cellpadding="0" class="normal_table">
        <tr>
          ...
          <td align="right" height="100px;" style="text-align:right;" >
            <form action="tags_poster.php" method="get" name="searchForm" id="searchForm">
              ...
              <?php }else{ 
                while (!$announcement->EOF) {
                  ...
                  <tr class="dataTableRow" onMouseOver="rowOverEffect(this)" onMouseOut="rowOutEffect(this)" ondblclick="window.location.href='<?php echo zen_href_link(FILENAME_CUSTOMER_SUGGESTION, 'page=' . $_GET['page'] . '&sID=' . $announcement->fields['an_id']). "&action=view_info";?>' ">                
                    ...
                    <a href="<?php echo zen_href_link('tags_poster', zen_get_all_get_params(array('action', 'id')) . 'action=create&anId=' . $announcement->fields['an_id']);?>"><?php echo zen_image(DIR_WS_IMAGES . 'icon_edit.gif', ICON_EDIT);?></a>
                    </td>
                  </tr>
                  <?php
        ...
        <table border="0" width="100%" cellspacing="0" cellpadding="2">
          ...
          <tr><a href="<?php echo zen_href_link('tags_poster', zen_get_all_get_params(array('action')) . 'action=create', 'NONSSL');?>"><button>新建标语</button></a></tr>
        </table>
