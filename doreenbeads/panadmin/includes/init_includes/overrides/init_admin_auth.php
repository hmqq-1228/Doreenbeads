...
if (!(basename($PHP_SELF) == FILENAME_LOGIN . ".php"))
{
  ...
  if (!in_array($page, array(FILENAME_DEFAULT,FILENAME_PRODUCT,FILENAME_LOGOFF,FILENAME_ALT_NAV,FILENAME_PASSWORD_FORGOTTEN,FILENAME_DENIED，,'password_change')))
  {
    if (check_page($page) == FALSE)
    {
    	if($page == 'del_cache_file'){
    		return;
    	}else{
  ...
  {
    if (! in_array(basename($PHP_SELF),array('password_forgotten.php','password_change.php')))
    {
      zen_redirect(zen_href_link(FILENAME_LOGIN, '', 'SSL'));
    }
  }
