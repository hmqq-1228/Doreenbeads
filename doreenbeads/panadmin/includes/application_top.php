...
if(!$check_result){
	...
	header("Location: ".HTTP_SERVER."/403.html");
	exit;
}
if((isset($_SERVER['REQUEST_SCHEME']) || isset($_SERVER['HTTP_X_FORWARDED_PROTO'])) && $_SERVER['REQUEST_SCHEME'] . '://' . $_SERVER['HTTP_HOST'] != HTTP_SERVER && $_SERVER['HTTP_X_FORWARDED_PROTO'] . '://' . $_SERVER['HTTP_HOST'] != HTTP_SERVER) {
	header("HTTP/1.1 302 Object Moved");
	header("location: ". HTTP_SERVER . $_SERVER['REQUEST_URI']);
	exit;
}
//echo DIR_FS_CATALOG . 'includes/smarty_config.php' ;die();
//include(DIR_FS_CATALOG . 'includes/smarty_config.php');
