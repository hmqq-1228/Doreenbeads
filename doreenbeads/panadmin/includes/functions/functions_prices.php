...
// compute discount based on qty
  function zen_get_products_discount_price_qty($product_id, $check_qty, $check_amount=0,$use_special_price = true) {
    global $db;
      $product_id = (int)$product_id;
      $ldc_special_discount = 0;
...
//   	if(zen_is_promotion_time()){
  		...
  		if($promotion_discount->fields['promotion_discount']>0){
  			$show_sale_discount=(int)$promotion_discount->fields['promotion_discount'];
  		}
//   	}
   
  	if($show_sale_discount==''){
  		$product = $db->Execute("select products_price from " . TABLE_PRODUCTS . " where products_id = '" . (int)$product_id . "' limit 1");
  		if ($product->RecordCount() > 0) {
  			$product_price = $product->fields['products_price'];
  		} else {
  			return false;
  		}
  		$sale = $db->Execute("Select sale_deduction_value, sale_specials_condition, sale_deduction_type
							  ...
							  And (sale_pricerange_to >= '" . $product_price . "'
							   or sale_pricerange_to = '0') order by sale_deduction_value desc limit 1");
  
  
  		if($sale->RecordCount() > 0) {
  			$show_sale_discount = (int)$sale->fields['sale_deduction_value'];;
  		}else{
  			$show_sale_discount = '';
  		}
  	}
