...
{
  $countries = array('USD', 'EUR', 'AUD', 'CAD', 'GBP', 'JPY', 'NZD', 'RUB', 'SGD', 'UAH', 'CNY');
  $curries_array = array();

  $a_parm['app'] = 'finance.rate';
  $a_parm['scur'] = 'USD';
  $a_parm['appkey'] = CURRENCY_API_KEY;
  $a_parm['sign'] = CURRENCY_API_SIGN;
  $a_parm['format'] = 'json';
  $a_parm['cur_date'] = date('Ymd');
  ...
  foreach ($countries as $cur) {
    ...
    if ($a_cdata['success'] != '1') {
      ...
      $curries_array[$cur] = $a_cdata['result']['rate'];

    }

  }
  return $curries_array;
}
//    function quote_xe_currency() {
//
//	$countries = array('USD','EUR','AUD','CAD','GBP','JPY','NZD','RUB','SGD','UAH');
//	foreach($countries as $to){
//		$url  = 'http://qq.ip138.com/hl.asp?from=USD&to='.$to.'&q=1';
//		$page = @file($url);
//		$htmlstr = str_replace('<td>',',',$page[481]);
//		$htmlstr = strip_tags($htmlstr);
//		$temparr = explode(',',$htmlstr);
//
//		$return[$to] = $temparr[2];
//
//	}
...
//     preg_match('/[0-9.]+( )*' . $from . '( )*=( )*([0-9.]+)( )*' . $to . '/', implode('', $page), $match);
//      if (sizeof($match) > 0) {
//       // return $match[1];
//       return $match[4];
//      } else {
