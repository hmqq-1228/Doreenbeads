...
function display_role_profiles($headers, $roleID) {
    global $db;

    while (!$headers->EOF){ //角色权限界面
        $sql = "SELECT * FROM ".TABLE_ADMIN_ROLE_HEADER_PRIVILEGE." WHERE header_id = '".$headers->fields['id']."' AND role_id = '".$roleID."'";
        $visible = $db->Execute($sql);
        if ($headers->fields['id'] == 0) {
            echo '<div class="menuHeaderOff"><span class="menuTitle">';
            echo $headers->fields['header'].TEXT_CONTRIBUTION_GUIDANCE;
        } else {
            if ($visible->fields['role_id']==$roleID) {
                echo '<div class="menuHeaderOn">'."\n";
                echo "\t".'<span class="menuTitle">';
                echo '<label><input name="'.$headers->fields['header'].'" type="checkbox" checked="on" value="on">';
            } else {
        ...
        while (!$pages->EOF) {
            echo "\t".'<label class="permissionBox"><input name="'.$pages->fields['page'].'" class="'.$headers->fields['header'].'" type="checkbox" ';
            $sql = "SELECT * FROM ".TABLE_ADMIN_ROLE_PAGE_PRIVILEGE." WHERE page_id = '".$pages->fields['id']."' AND role_id = '".$roleID."'";
            $allow = $db->Execute($sql);
            if ($allow->fields['role_id']==$roleID) {
                echo 'checked="on" value="on">';
            } else {
                echo 'value="off">';
            }
...
function admin_update_or_add_role($admins_id, $role_id, $update = false, $new_role_id){
    global $db;
    
    if($update){
        if($role_id > 0){
            $admin_allowed_header_sql="SELECT role_id,header_id FROM ".TABLE_ADMIN_ROLE_HEADER_PRIVILEGE." WHERE role_id=".$role_id;
            $admin_allowed_header=$db->Execute($admin_allowed_header_sql);
            while(!$admin_allowed_header->EOF){
                $role_allowed_header_sql = "DELETE FROM ".TABLE_ADMIN_VISIBLE_HEADERS." WHERE header_id = ".$admin_allowed_header->fields['header_id']." AND admin_id = :adminId:";
                $bind_role_allowed_header_sql = $db->bindVars($role_allowed_header_sql, ':adminId:', $admins_id, 'integer');
                $db->Execute($bind_role_allowed_header_sql);
                $admin_allowed_header->MoveNext();
            }
            
            $admin_allowed_page_sql="SELECT role_id,page_id FROM ".TABLE_ADMIN_ROLE_PAGE_PRIVILEGE." WHERE role_id=".$role_id;
            $admin_allowed_page=$db->Execute($admin_allowed_page_sql);
            while(!$admin_allowed_page->EOF){
                $role_allowed_page_sql = "DELETE FROM ".TABLE_ADMIN_ALLOWED." WHERE page_id = '".$admin_allowed_page->fields['page_id']."' AND admin_id = :adminId:";
                $bind_role_allowed_page_sql = $db->bindVars($role_allowed_page_sql, ':adminId:', $admins_id, 'integer');
                $db->Execute($bind_role_allowed_page_sql);
                $admin_allowed_page->MoveNext();
            }
        }
        $role_id = $new_role_id;
    }
    ...
    $admin_allowed_header=$db->Execute($admin_allowed_header_sql);
    while(!$admin_allowed_header->EOF){
        $role_allowed_header_sql = "INSERT INTO ".TABLE_ADMIN_VISIBLE_HEADERS." SET header_id = ".$admin_allowed_header->fields['header_id'].", admin_id = :adminId:";
        $bind_role_allowed_header_sql = $db->bindVars($role_allowed_header_sql, ':adminId:', $admins_id, 'integer');
        $db->Execute($bind_role_allowed_header_sql);
        $admin_allowed_header->MoveNext();
    }
    ...
    $admin_allowed_page=$db->Execute($admin_allowed_page_sql);
    while(!$admin_allowed_page->EOF){
        $role_allowed_page_sql = "INSERT INTO ".TABLE_ADMIN_ALLOWED." SET page_id = ".$admin_allowed_page->fields['page_id'].", admin_id = :adminId:";
        $bind_role_allowed_page_sql = $db->bindVars($role_allowed_page_sql, ':adminId:', $admins_id, 'integer');
        $db->Execute($bind_role_allowed_page_sql);
        $admin_allowed_page->MoveNext();
    }
