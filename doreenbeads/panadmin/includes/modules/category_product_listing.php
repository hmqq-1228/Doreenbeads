      ...
      <tr>
        <td><table border="0" width="100%" cellspacing="0" cellpadding="0">
          <tr>
            ...
            <td><button id="clear_categories_file_cache">clear categories file cache</button></td>
...
<?php  if ($action == '') { ?>
              ...
              <tr>
                <td colspan="3"><table border="0" width="100%" cellspacing="0" cellpadding="2">
                  ...
                  <tr>
                    <td class="smallText" style="float:right;">
                    <?php if($_GET['cPath'] == '718_356'){
                    		echo zen_draw_form('quantity_form', FILENAME_CATEGORIES, '', 'post','enctype="multipart/form-data"');?>
		                    	Change the quantity of product file:<input type="file" name="file_quantity"/>
		                    	<input type="hidden" name="action" value="upload_quantity">
		                    	 <input type="hidden" name="cPath" value="<?php echo $cPath; ?>">
		                    	<input  type="submit"  name="submit_quantity" style="width:75px;font-size:13px;" value="upload" />
	                    	</form>
	                <?php }?>
                    </td>
                    
                    <td align="left" class="smallText" style="padding-left:20%"> <?php 
                    echo $show_pro_no_cate?'<input onclick="location.href=\''.zen_href_link(FILENAME_CATEGORIES,'cID=' . $current_category_id.'&action=viewProNoCate').'\'" style="cursor:pointer;" type="button" value="show no category products">':'';
                    ?>
                ...
                ?>
                <?php echo '&nbsp;&nbsp;' . zen_draw_pull_down_menu('product_type', $product_restrict_types_array , '', 'class="product_type"');
                echo zen_hide_session_id(); ?>
                           <input type="hidden" name="cPath" value="<?php echo $cPath; ?>">
...
<?php 
 switch ($_SESSION['categories_products_sort_order']) {
    ...
    $products_count = 0;
    if (isset($_REQUEST['search']) && zen_not_null($_REQUEST['search'])) {
      $search_array = split(',', trim($_REQUEST['search']));
      $search_string = '';
      for($i=0;$i<sizeof($search_array);$i++){
      	if(trim($search_array[$i]) !='')$search_string.="'".trim($search_array[$i])."',";
      }
      $search_string = substr($search_string, 0, strlen($search_string)-1);
     // echo $search_string;
	  if($_POST['showCategory']=='no'){
	  	$products_query_raw = ("select p.products_id, p.products_type,  pd.products_name,
      									pd.products_name_without_catg, 
                                       p.products_image, p.products_price, p.products_net_price, p.products_date_added,
                                ...
                                and ( p.products_model in (" . $search_string . "))  group by p.products_id" .
                                $order_by);
				
	  }else{
	  	$products_query_raw = ("select distinct p.products_id, p.products_type,  pd.products_name, 
      									pd.products_name_without_catg, 
                                       p.products_image, p.products_price, p.products_net_price, p.products_date_added,
                                       p.products_last_modified, p.products_date_available,
                                       p.products_status, p2c.categories_id,
                                ...
                                and (
                                pd.products_name like '%" . zen_db_input(trim($_REQUEST['search'])) . "%'                               
                                or p.products_model in (" . $search_string . "))" .
                                $order_by);
	  }
...
//      zen_redirect(zen_href_link(FILENAME_CATEGORIES, 'cPath=' . $_GET['cPath'] . '&pID=' . $_GET['pID'] . (isset($_GET['page']) ? '&page=' . $_GET['page'] : '')));
    ...
    //echo $products_query_raw;
    //echo MAX_DISPLAY_RESULTS_CATEGORIES;
   // echo $products_query_numrows;
    //echo $products->RecordCount();
?>
      <tr>
        <td><table border="0" width="100%" cellspacing="0" cellpadding="0">
          <tr>
...
<?php  if ($action == '') { 
	...
	?>
     <?php
     if ($zc_skip_products == false || (isset($_GET['search'])&&$products->RecordCount()>0)) { ?>         
			   <td class="dataTableHeadingContent"  width="10" ><input type="checkbox" onclick="getAllCheck(this,'productcheckbox[]')"></td>
			   <?php }?>
                ...
                <td class="dataTableHeadingContent" align="right">上货价</td>
                <td class="dataTableHeadingContent" align="center"><?php echo 'Image'; ?></td>
...
<?php if ($action == '') { ?>
                ...
                <td class="dataTableContent" align="center"><?php echo zen_image(DIR_WS_CATALOG.DIR_WS_IMAGES.$categories->fields['categories_image'],'',100,73 ) ?></td>
...
?>
                ...
                <td class="dataTableContent" align="right">
                  ...
                  <?php if(count(zen_parse_category_path($_GET['cPath'])) < 6){
                  		 echo '<a href="' . zen_href_link(FILENAME_CATEGORIES, 'cPath=' . $cPath . '&cID=' . $categories->fields['categories_id'] . '&action=move_category') . '">' . zen_image(DIR_WS_IMAGES . 'icon_move.gif', ICON_MOVE) . '</a>';
						} ?>
<?php
...
?> 
      ...
      ?>
      <?php if($_POST['showCategory']=='no'){
      	$formOther='diffUrl=yes&';
      }?>
   <form id="productChange" action="<?php echo zen_href_link('product.php', $formOther.'cPath=' . $cPath . '&product_type=' . $products->fields['products_type'].$link_url . $currentPage, 'NONSSL'); ?>" method = "post" onsubmit="return changeAction1()" > 
<?php 
      }
      
    while (!$products->EOF) {
	/*products_stock WSL*/
	 $products_quantity = get_products_quantity(array('products_id'=>$products->fields['products_id']));
	 $products->fields['products_quantity'] = $products_quantity;
	 
      $products_count++;
      $rows++;

// Get categories_id for product if search
      if (isset($_REQUEST['search'])) $cPath = $products->fields['categories_id'];
...
// Split Page
      ...
      <?php 
        // zen_href_link($type_handler, 'cPath=' . $cPath . '&product_type=' . $products->fields['products_type'] . '&action=moveAll'
...
//      }
// Split Page
      if(isset($_POST['productcheckbox'])&&$_POST['productcheckbox']!=''&&in_array(($products->fields['products_id']),$_POST['productcheckbox'])){
 		echo '<tr class="dataTableRow" style="background-color:#F6F6D4;" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)" >' . "\n";
      }else{
...
//}?>
          ...
          ?>
				<td class="dataTableContent" width="10"><input type="checkbox" name="productcheckbox[]" value="<?php echo $products->fields['products_id'].$otherStr;?>" <?php echo $input_check; ?> ></td>
			<?php }?>
                <td class="dataTableContent" width="20" align="right"><?php echo $products->fields['products_id']; ?></td>
<!--add without_catg display at here robbie-->
                <td class="dataTableContent"><?php echo '<a href="' . zen_href_link(FILENAME_PRODUCT, 'cPath=' . $cPath . '&pID=' . $products->fields['products_id'] . '&action=new_product_preview&read=only' . '&product_type=' . $products->fields['products_type'] . (isset($_GET['page']) ? '&page=' . $_GET['page'] : '')) . '">' . zen_image(DIR_WS_ICONS . 'preview.gif', ICON_PREVIEW) . '</a>&nbsp;' . $products->fields['products_name'] . '<br />' . $products->fields['products_name_without_catg']; ?></td>
                <td class="dataTableContent"><?php echo $products->fields['products_model']; ?></td>
                <td colspan="1" class="dataTableContent" align="right"><?php echo $products->fields['products_net_price']; ?></td>
				<td class="dataTableContent">
					<?php 
						$img_extension = substr($products->fields['products_image'], strrpos($products->fields['products_image'], '.'));
						$src = str_replace($img_extension, 'A_80_80.JPG', $products->fields['products_image']);
					echo '<img src="'.HTTP_IMG_SERVER.'bmz_cache/watermarkimg_new/'.$src.'"/>';
...
<?php
	  ...
	  echo zen_draw_hidden_field('product_status['.$products->fields['products_id'].']', $products->fields['products_status']);
      echo zen_draw_hidden_field('product_cPath['.$products->fields['products_id'].']', $cPath);
...
<?php if ($action == '') { ?>
				...
				<a href="<?php echo zen_href_link(FILENAME_CATEGORIES, zen_get_all_get_params().'pID='.$products->fields['products_id']); ?>">更新前台显示图片</a><br/><br/>
				<?php }?>
        ...
        <?php if(count(zen_parse_category_path($_GET['cPath'])) < 6){echo '<a href="' . zen_href_link($type_handler, 'cPath=' . $cPath . '&product_type=' . $products->fields['products_type'] . '&pID=' . $products->fields['products_id'] . '&action=move_product') . '">' . zen_image(DIR_WS_IMAGES . 'icon_move.gif', ICON_MOVE) . '</a>';} ?>
...
<?php
	...
	echo zen_draw_hidden_field('currentPage', (isset($_GET['page']) ? $_GET['page'] : 1)); 
              ?>
              <?php if(($action == '' && $zc_skip_products == false)||($_GET['search']&&$products->RecordCount()!=0&&$action == '')){?>
              <tr>
              <td colspan="4">&nbsp;</td>
              <td colspan="6" align="right">
	              <div>
		              <input type="submit" id="moveAll"  onclick="changeAction1(this)" value="<?php echo TEXT_MOVE_SELECTED?>">
		              <input type="submit" id="copyAll" onclick="changeAction1(this)" value="<?php echo TEXT_COPY_SELECTED?>">
		              <input type="submit" id="disableAll" onclick="changeAction1(this)" value="<?php echo TEXT_DISABLE_SELECTED?>">
		              <input type="submit" id="listAll" onclick="changeAction1(this)" value="List Selected">
		              <input type="submit" id="unListAll" onclick="changeAction1(this)" value="Terminate Selected">
	              </div>
	          </td>
              	</tr>
...
<?php
    ...
    echo '<br>'.CATEGORY_HAS_SUBCATEGORIES;
?>
<?php } // hide has cats?>
          &nbsp;</td>
                  </tr>
...
<script src="includes/javascript/jscript_jquery.js"></script>            
<script type="text/javascript">
$("#clear_categories_file_cache").click(function(){
	$.post('./del_cache_file.php',function(data){
		alert(data);		
	});
	//unlink( DIR_FS_CATALOG . 'fileCache/');
})
</script>            
