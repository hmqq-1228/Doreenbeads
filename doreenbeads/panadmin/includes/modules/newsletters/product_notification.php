...
//--></script>';
    ...
    function send($newsletter_id, $product_id = 'robbie') {
      global $_POST, $db;
      ...
      if (isset($_POST['global']) && ($_POST['global'] == 'true')) {
		...
		//Product Notification get the product infomation
        //robbie wei
        $products = $db->Execute("Select pd.products_id, products_name, products_model, products_image 
							From " . TABLE_PRODUCTS . " as p, " . TABLE_PRODUCTS_DESCRIPTION . " as pd 
							Where p.products_id = pd.products_id And p.products_id in (" . $ids . ")");
        if ($products->RecordCount() >= 1) {
        	...
        	$products_link = HTTP_SERVER . "/index.php?main_page=product_info&products_id=" . $product_id;
        	$notify_mana_link = HTTP_SERVER . "/index.php?main_page=account_notifications";
        	$products_image = HTTP_SERVER . "/images/" . $products->fields['products_image'];
        }
        //eof robbie
        
        //product notification  Cancel the Global notification
//        $customers = $db->Execute("select c.customers_id, c.customers_firstname, c.customers_lastname,
//                                          c.customers_email_address
//                                   from " . TABLE_CUSTOMERS . " c, " . TABLE_CUSTOMERS_INFO . " ci
//                                   where c.customers_id = ci.customers_info_id
...
//send emails
      reset($audience);
    $i=0;
//    $this->title = $this->title . " [" . $products_name . "]";
//    print_r($audience);
      ...
      $this->title = str_replace("[products name]", $products_name, $this->title);
      $this->title = str_replace("[products model]", $products_model, $this->title);
      $html_msg['EMAIL_MESSAGE_HTML'] = str_replace("[products name]", $products_name, $html_msg['EMAIL_MESSAGE_HTML']);
      $html_msg['EMAIL_MESSAGE_HTML'] = str_replace("[products image]", $products_image, $html_msg['EMAIL_MESSAGE_HTML']);
      $html_msg['EMAIL_MESSAGE_HTML'] = str_replace("[notify mana]", $notify_mana_link, $html_msg['EMAIL_MESSAGE_HTML']);
      $html_msg['EMAIL_MESSAGE_HTML'] = str_replace("[server]", HTTP_SERVER, $html_msg['EMAIL_MESSAGE_HTML']);
      $html_msg['EMAIL_MESSAGE_HTML'] = str_replace("[products link]", $products_link, $html_msg['EMAIL_MESSAGE_HTML']);
      $html_msg['EMAIL_MESSAGE_HTML'] = str_replace("[products model]", $products_model, $html_msg['EMAIL_MESSAGE_HTML']);
      if ($product_id <> 'robbie') $html_msg['EMAIL_MESSAGE_HTML'] = str_replace("[products id]", 
      										$product_id, $html_msg['EMAIL_MESSAGE_HTML']);
      $this->content = str_replace("[products name]", $products_name, $this->content);
      $this->content = str_replace("[products model]", $products_model, $this->content);
      if ($product_id <> 'robbie') $this->content = str_replace("[products id]", $product_id, $this->content);
