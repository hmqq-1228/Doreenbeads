...
if (zen_not_null($_POST)) {
    ...
    $product = $db->Execute("select p.products_id, pd.language_id, pd.products_name,
                                      ...
                                      p.products_weight, p.products_volume_weight, p.products_date_added, p.products_last_modified,
                                      p.products_date_available, p.products_status, p.manufacturers_id,
                                      p.products_quantity_order_min, p.products_quantity_order_units, p.products_priced_by_attribute,
                                      p.product_is_free, p.product_is_call, p.products_quantity_mixed,
                                      p.product_is_always_free_shipping, p.products_qty_box_status, p.products_quantity_order_max,
                   					  p.products_sort_order, p.products_stocking_days
...
for ($i = 0, $n = sizeof($languages); $i < $n; $i++) {
    if (isset($_GET['read']) && ($_GET['read'] == 'only')) {
        ...
        $pInfo->products_name_without_catg = zen_db_prepare_input($products_name_without_catg[$languages[$i]['id']]);
    ...
    <tr>
        <td class="main">
            ...
            if ($_POST['image_delete'] == 1 || $products_image_name == '' and PRODUCTS_IMAGE_NO_IMAGE_STATUS == '1') {
                ...
                $img_extension = substr($products_image_name, strrpos($products_image_name, '.'));
                $src = str_replace($img_extension, 'A.jpg', $products_image_name);
                echo zen_image(HTTP_SERVER . '/download/' . $src, $pInfo->products_name, SMALL_IMAGE_WIDTH, SMALL_IMAGE_HEIGHT, 'align="right" hspace="5" vspace="5"') . $pInfo->products_description;
            }
            ?>
        </td>
    </tr>
    ...
    if ($pInfo->products_date_available > date('Y-m-d')) {
        ...
        <tr>
            <td align="center"
                class="smallText"><?php echo sprintf(TEXT_PRODUCT_DATE_AVAILABLE, zen_date_long($pInfo->products_date_available)); ?></td>
        </tr>
        <?php
    } else {
        ?>
        <tr>
            <td align="center"
                class="smallText"><?php echo sprintf(TEXT_PRODUCT_DATE_ADDED, zen_date_long($pInfo->products_date_added)); ?></td>
        </tr>
        <?php
    }
    ?>
...
if (isset($_GET['read']) && ($_GET['read'] == 'only')) {
    ...
    <tr>
        <td align="right" class="smallText">
            ...
            while (list($key, $value) = each($_POST['property_value'])) {
                echo zen_draw_hidden_field('property_group_id[' . $key . ']', $value);
            }

            $languages = zen_get_languages();
            for ($i = 0, $n = sizeof($languages); $i < $n; $i++) {
                echo zen_draw_hidden_field('products_name[' . $languages[$i]['id'] . ']', htmlspecialchars(stripslashes($products_name[$languages[$i]['id']])));
                echo zen_draw_hidden_field('products_name_without_catg[' . $languages[$i]['id'] . ']', htmlspecialchars(stripslashes($products_name_without_catg[$languages[$i]['id']])));
            ...
            echo zen_draw_hidden_field('products_stocking_days', $pInfo->products_stocking_days);
            echo zen_draw_hidden_field('is_display', $pInfo->is_display);
