...
// Editor alerts
  if (HTML_EDITOR_PREFERENCE != 'NONE' && !is_dir('editors')) {
...
<?php } // alt_nav spacing ?>
  <tr>
    <td align="left" valign="top" height="0" width="<?php echo HEADER_LOGO_WIDTH; ?>"><?php echo '<a href="' . zen_href_link(FILENAME_DEFAULT) . '">' . zen_image(DIR_WS_IMAGES . 'logo1.png', HEADER_ALT_TEXT, '', '', 'style="margin: 15px;"') . '</a>'; ?></td>
...
<table border="0" cellspacing="0" cellpadding="0" width="100%">
  <tr class="headerBar" height="40" width="100%">
    ...
    for ($i = 0, $n = sizeof($langs); $i < $n; $i++) {
        ...
        if(basename($PHP_SELF) == 'categories.php'){ 
 		       echo DEFINE_LANGUAGE . '&nbsp;&nbsp;' . (sizeof($langs) > 1 ? zen_draw_pull_down_menu('language', $langs_array, $langs_selected, 'onChange="this.form.submit();"') : ''); 
 		                $params_arr=explode('&',zen_get_all_get_params(array('language'))); 
 		                if(sizeof($params_arr)>0){ 
 		                        foreach($params_arr as $param){ 
 		                                if($param!=''){ 
 		                                        $param_arr=explode('=', $param); 
 	                                echo '<input type="hidden" name="'.$param_arr[0].'" value="'.$param_arr[1].'">'; 
 		                                } 
 		                        } 
 		                } 
	    } else{
    ...
    <td class="headerBarContent" align="right"><?php echo 
    		'<a href="' . zen_href_link(FILENAME_DEFAULT, '', 'NONSSL') . '" class="headerLink">' . HEADER_TITLE_TOP . '</a>&nbsp;|&nbsp;
 			 ...
 			 <a href="http://www.zen-cart.com/" class="headerLink" target="_blank">' . HEADER_TITLE_SUPPORT_SITE . '</a>&nbsp;|&nbsp;
  			 ...
  			 <a href="' . zen_href_link('password_change') . '" class="headerLink">修改密码</a>&nbsp;|&nbsp;
			 <a href="' . zen_href_link(FILENAME_LOGOFF) . '" class="headerLink">' . HEADER_TITLE_LOGOFF . '</a>&nbsp;'; ?></td>
  </tr>
</table>
