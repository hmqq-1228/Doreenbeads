  ...
  if (zen_not_null($action)) {
    switch ($action) {
      ...
      case 'setflag':
        ...
        break;
      case 'insert':
      	////jessa 2010-04-23 ��ݲ�Ʒ�ı�����޸�featured��Ʒ
      	if ($_POST['products_model'] == ''){
      		$messageStack->add_session(ERROR_NOTHING_SELECTED, 'caution');
      	} else {
      		$products_models = zen_db_prepare_input($_POST['products_model']);
      		$products_models_array = explode(',', $products_models);
      		$products_models_format = '';
      		for ($pm_cnt = 0; $pm_cnt < sizeof($products_models_array); $pm_cnt++){
      			$products_models_format .= '\'' . $products_models_array[$pm_cnt] . '\',';
      		}
      		$products_models = substr($products_models_format, 0, -1);
      		
      		$get_products_id = $db->Execute("Select products_id
        									   From " . TABLE_PRODUCTS . "
        									  Where products_model in (" . $products_models . ")");
      		...
      		while (!$get_products_id->EOF){
				$products_id = $get_products_id->fields['products_id'];
				...
				$get_products_id->MoveNext();
			}
      	}
      	//eof jessa 2010-04-23
        if ($_GET['go_back'] == 'ON'){
...
//        bof removeselectconfirm
     ...
     // demo active test
        ...
        $chooseConfirm=$_POST['chooseConfirm'];
       if($chooseConfirm==""){
       	zen_redirect(zen_href_link(FILENAME_FEATURED, 'page=' . $_GET['page']));
       }else{
       	$aChoose=explode(';',$chooseConfirm );
       	foreach ($aChoose as $chooseid){
       		$db->Execute("delete from " . TABLE_FEATURED . "
                      where featured_id = '" . (int)$chooseid. "'");
       		
       	}
       	 zen_redirect(zen_href_link(FILENAME_FEATURED, 'page=' . $_GET['page']));
       }
	break;
//	eof removeselectconfirm
      case 'removeconfirm':
        // demo active test
        if (zen_admin_demo()) {
          $_GET['action']= '';
        ...
        zen_redirect(zen_href_link(FILENAME_FEATURED));
        break;
...
?>
                  <td align="center">
                  <?php echo $_POST['']?>
                    <?php echo '<a href="' . zen_href_link(FILENAME_FEATURED, ((isset($_GET['page']) && $_GET['page'] > 0) ? 'page=' . $_GET['page'] . '&' : '') . 'action=new') . '">' . zen_image_button('button_new_product.gif', IMAGE_NEW_PRODUCT) . '</a>'; ?>
                    <?php echo '<a href="' . zen_href_link(FILENAME_FEATURED, 'page=' . $_GET['page'] . '&action=remove') . '">' . zen_image_button('button_remove.gif', IMAGE_REMOVE) . '</a>'; ?>                   
                   <?php echo zen_image_button('button_remove_select.gif','remove_select','onclick="removeSelect()"')?>
                  </td>
<?php
  }
?>
      ...
      <tr><form name="new_featured" <?php echo 'action="' . zen_href_link(FILENAME_FEATURED, zen_get_all_get_params(array('action', 'info', 'fID')) . 'action=' . $form_action . '&go_back=' . $_GET['go_back'], 'NONSSL') . '"'; ?> method="post"><?php if ($form_action == 'update') echo zen_draw_hidden_field('featured_id', $_GET['fID']); ?>
        <td><br><table border="0" cellspacing="0" cellpadding="2">
          <tr>
            ...
            <td class="main"><?php echo (isset($fInfo->products_name)) ? $fInfo->products_name . ' <small>(' . $currencies->format($fInfo->products_price) . ')</small>' : zen_draw_input_field('products_model', '', 'size="45"');?><?php echo (isset($fInfo->products_name)) ? '' : ' <span style="color:red;">Input products model like (B00001,B00002,B00003)</span>'; echo zen_draw_hidden_field('products_price', (isset($fInfo->products_price) ? $fInfo->products_price : ''));?></td>
            <!--<td class="main">
                  <?php 
                     //echo (isset($fInfo->products_name)) ? $fInfo->products_name . ' <small>(' . $currencies->format($fInfo->products_price) . ')</small>' : zen_draw_products_pull_down('products_id', 'size="5" style="font-size:12px"', $featured_array, true, $_GET['add_products_id'], true); 
          ...
          <tr>
            <td class="main"><?php echo zen_draw_hidden_field('end',''); ?></td>
            <td class="main"></td>
          </tr>
        </table></td>
      </tr>
...
?>
      <tr>
        <td><table border="0" width="100%" cellspacing="0" cellpadding="0">
          <tr>
            <td valign="top">
            <form id='removeform' action="<?php echo HTTP_SERVER?>/8admin/featured.php?page=1&action=remove_select" name="removeform" method="post">
            <table border="0" width="100%" cellspacing="0" cellpadding="2">
              <tr class="dataTableHeadingRow">
             <?php  if (empty($action)) {
               echo  '<td class="dataTableHeadingContent" width="50px">All<input type="checkbox" name="chooseAll" id="chooseAll" onchange="selectAll();"/>&nbsp;&nbsp;&nbsp;&nbsp;</td>';
             }?>   
...
// create split page control
    ...
    while (!$featured->EOF) {
      ...
      if (isset($fInfo) && is_object($fInfo) && ($featured->fields['featured_id'] == $fInfo->featured_id)) {
        echo '                  <tr id="defaultSelected" class="dataTableRowSelected" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)" ondblclick="document.location.href=\'' . zen_href_link(FILENAME_FEATURED, 'page=' . $_GET['page'] . '&fID=' . $fInfo->featured_id . '&action=edit') . '\'">' . "\n";
      } else {
        echo '                  <tr class="dataTableRow" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)" ondblclick="document.location.href=\'' . zen_href_link(FILENAME_FEATURED, 'page=' . $_GET['page'] . '&fID=' . $featured->fields['featured_id'] . '&action=edit') . '\'">' . "\n";
      }
...
<?php 
 if (empty($action)) {
             echo   '<td  class="dataTableContent" >&nbsp;&nbsp;&nbsp;
                <input type="checkbox" name="choose[]" value="'. $featured->fields['featured_id'].'"/></td>';
...
?>
                  <tr>
                    <td align="right" colspan="2" width="80%"><?php echo '<a href="' . zen_href_link(FILENAME_FEATURED, 'page=' . $_GET['page'] . '&action=new') . '">' . zen_image_button('button_new_product.gif', IMAGE_NEW_PRODUCT) . '</a>'; ?>
                   <?php echo '<a href="' . zen_href_link(FILENAME_FEATURED, 'page=' . $_GET['page'] . '&action=remove') . '">' . zen_image_button('button_remove.gif', IMAGE_REMOVE) . '</a>'; ?>
                     <?php echo zen_image_button('button_remove_select.gif','remove_select','onclick="removeSelect()"')?>
                    </td>
                  </tr>
...
<?php
  ...
  switch ($action) {
  	case 'remove_select':
    	$removeIds='';
  	foreach ($_POST['choose'] as $removeId) {
  		$removeIds.=$removeId.';';
  	}
  	  $heading[]=array('text' =>'<b>remove selected</b>');
  	  $contents = array('form' => zen_draw_form('featured', FILENAME_FEATURED, 'action=removeSelectConfirm'));
      $contents[] = array('text' => '<b>WARNING:</b> <font color="red">Are you sure you want to delete all the selected featured?</font>');
      $contents[] = array('text' => '<input type="hidden" name="chooseConfirm" value="'.$removeIds.'" /><br />');
      $contents[] = array('align' => 'center', 'text' => '<br />' . zen_image_submit('button_delete.gif', IMAGE_DELETE) . '&nbsp;<a href="' . zen_href_link(FILENAME_FEATURED, 'page=' . $_GET['page']) . '">' . zen_image_button('button_cancel.gif', IMAGE_CANCEL) . '</a>');
  		break;
    case 'delete':
      ...
      break;
    case 'remove':
      $heading[] = array('text' => '<b>' . TEXT_INFO_HEADING_DELETE_ALL . '</b>');

      $contents = array('form' => zen_draw_form('featured', FILENAME_FEATURED, 'action=removeconfirm'));
      $contents[] = array('text' => TEXT_DELETE_ALL_FEATURED);
      $contents[] = array('align' => 'center', 'text' => '<br />' . zen_image_submit('button_delete.gif', IMAGE_DELETE) . '&nbsp;<a href="' . zen_href_link(FILENAME_FEATURED, 'page=' . $_GET['page']) . '">' . zen_image_button('button_cancel.gif', IMAGE_CANCEL) . '</a>');
      break;
    case 'pre_add':
