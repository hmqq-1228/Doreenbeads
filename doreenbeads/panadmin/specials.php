  ...
  if (zen_not_null($action)) {
    switch ($action) {
      case 'setflag':
        ...
        break;
      case 'insert':
      	////jessa 2010-04-23 根据产品的编号来添加specials产品
      	if ($_POST['products_model'] == ''){
      		...
      		$products_id_split = explode(',', $products_id);
      		for ($num = 0; $num < sizeof($products_id_split); $num++){
      			$products_id_split[$num] = substr($products_id_split[$num], 1);
      			$products_id_split[$num] = substr($products_id_split[$num], 0, -1);
	      		$db->Execute("insert into " . TABLE_SPECIALS . "
	      					  ...
	      					  values ('" . (int)$products_id_split[$num] . "',
	      					  '" . zen_db_input($specials_price) . "',
	      					  now(),
	      					  '" . zen_db_input($expires_date) . "', '1', '" . zen_db_input($specials_date_available) . "')");
	      		
	      		$new_special = $db->Execute("select specials_id from " . TABLE_SPECIALS . " where products_id='" . (int)$products_id_split[$num] . "'");
	      		zen_update_products_price_sorter((int)$products_id_split[$num]);
      		}
      	}
        ...
        <td><br><table border="0" cellspacing="0" cellpadding="2">
          <tr>
            ...
            <!--<td class="main"><?php //echo (isset($sInfo->products_name)) ? $sInfo->products_name . ' <small>(' . $currencies->format($sInfo->products_price) . ')</small>' : zen_draw_products_pull_down('products_id', 'size="5" style="font-size:10px"', $specials_array, true, $_GET['add_products_id'], true); echo zen_draw_hidden_field('products_price', (isset($sInfo->products_price) ? $sInfo->products_price : '')); ?></td>-->
          	<td class="main"><?php echo (isset($sInfo->products_name)) ? $sInfo->products_name . ' <small>(' . $currencies->format($sInfo->products_price) . ')</small>' : zen_draw_input_field('products_model', '', ' size="45"'); echo zen_draw_hidden_field('products_price', (isset($sInfo->products_price) ? $sInfo->products_price : ''));?><?php echo (isset($sInfo->products_name)) ? '' : ' <span style="color:red;">Input products model like (B00001,B00002,B00003)</span>'; ?></td>
          </tr>
          <tr>
